...
class TestRequirementsAnalyzerAcceptance:
    ...
    def _chain(self, analyzer):
        ...
        analyzer.add_requirements(
            [
                Requirement(
                    "A", "Alpha", "Alpha base requirement", RequirementType.FUNCTIONAL
                ),
                Requirement(
                    "B",
                    "Beta",
    ...
    def test_duplicate_requirement_rejected(self, analyzer):
        ...
        analyzer.add_requirement(
            Requirement("R1", "One", "description text", RequirementType.FUNCTIONAL)
        )
        with pytest.raises(ValueError, match="already exists"):
            analyzer.add_requirement(
                Requirement("R1", "Dup", "other text", RequirementType.DATA)
            )
...
class TestTraceabilityManagerAcceptance:
    ...
    def test_coverage_report_counts_traced_and_untraced(self, manager):
        ...
        manager.add_trace_links(
            [
                ...
                TraceLink(
                    "R2", "design.md", ArtifactType.DESIGN_DOCUMENT, verified=False
                ),
            ]
        )
    ...
    def test_trace_matrix_verification_status(self, manager):
        ...
        manager.add_trace_link(
            TraceLink("R1", "a.py", ArtifactType.SOURCE_CODE, verified=True)
        )
        manager.add_trace_link(
            TraceLink("R1", "b.py", ArtifactType.SOURCE_CODE, verified=True)
        )
        manager.add_trace_link(
            TraceLink("R2", "c.py", ArtifactType.SOURCE_CODE, verified=False)
        )
    ...
    def test_verify_link_flips_status(self, manager):
        ...
        manager.add_trace_link(
            TraceLink("R1", "a.py", ArtifactType.SOURCE_CODE, verified=False)
        )
...
class TestRequirementValidatorAcceptance:
    ...
    def test_clean_spec_set_is_consistent(self, validator):
        ...
        validator.add_specs(
            [
                RequirementSpec(
                    "S1", "Unique A", "Description one long enough", 2, 10.0
                ),
                RequirementSpec(
                    "S2", "Unique B", "Description two long enough", 3, 20.0
                ),
            ]
        )
        report = validator.check_consistency()
