...
class TestPlotRollingStatistics:
    ...
    def test_rolling_mean_only(self, viz, sample_values):
        ...
        window = 10
        rolling_mean = list(pd.Series(sample_values).rolling(window=window).mean().dropna())
        
        fig = viz.plot_rolling_statistics(
            sample_values, rolling_mean, window=window
        )
...
class TestCreateDashboard:
    ...
    def test_dashboard_with_forecast(self, viz, sample_values):
        ...
        forecast = {
            ...
            'lower': list(np.linspace(sample_values[-1] - 5, sample_values[-1] + 15, 10)),
            'upper': list(np.linspace(sample_values[-1] + 5, sample_values[-1] + 25, 10))
        }
