    ...
    def test_end_to_end_workflow(
        ...
        try:
            ...
            phenology = sa.identify_phenological_stages(
                crop_type="corn", variable="ndvi"
            )

            # Analyze temporal trends
            trends = sa.analyze_temporal_trends(variable="ndvi", period="daily")
            ...
            yield_model = CropYieldModel(
                crop_type="corn", model_type="machine_learning"
            )

            # Create training data with yield
            field_data_with_yield = fbm.fields.copy()
            field_data_with_yield["yield"] = np.random.uniform(
                5.0, 10.0, len(field_data_with_yield)
            )
            ...
            soil_results = soil_analysis.run(
                field_data=fbm.fields, soil_data=sample_soil_data
            )
            ...
            water_results = water_analysis.run(
                field_data=fbm.fields, weather_data=sample_weather_data
            )
            ...
            analysis_results.field_data.plot(
                ...
                if "predicted_yield" in analysis_results.field_data.columns
                else "field_id",
                legend=True,
            )
            ...
            sus.plot_sustainability_metrics(
                ax=axes[1, 1], metric_type="sustainability_index"
            )
            ...
            assert (
                len(fbm.fields) == len(sample_field_data) + 1
            )  # Original fields + new field
            ...
            assert "soil_health_index" in soil_results.results.get(
                "spatial_results", {}
            )
            assert "water_requirement_mm" in water_results.results.get(
                "spatial_results", {}
            )
