...
class TestCropYieldModel:
    ...
    def test_fit_machine_learning(self):
        ...
        model = CropYieldModel(crop_type="corn", model_type="machine_learning")
        model.fit(training_data={"field_data": field_data}, target_column="yield")
        ...
        with pytest.raises(ValueError):
            model.fit(
                training_data={"field_data": field_data_no_yield}, target_column="yield"
            )
    ...
    def test_predict_process_based(self):
        ...
        with pytest.raises(ValueError):
            model.predict(
                data={"field_data": test_fields, "weather_data": weather_data}
            )
