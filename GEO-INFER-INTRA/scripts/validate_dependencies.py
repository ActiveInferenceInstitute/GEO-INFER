        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if (
                        module_name not in STDLIB_MODULES
                        and module_name not in GEO_INFER_MODULES
                    ):
...
def check_version_compatibility(
    all_deps: Dict[str, List[Tuple[str, Optional[str]]]],
) -> Dict[str, List[str]]:
...
def analyze_module(module_path: Path, module_name: str) -> Dict:
    ...
    inconsistencies = []
    all_declared = (
        set(req_deps.keys()) | set(setup_deps.keys()) | set(pyproject_deps.keys())
    )
    for pkg in all_declared:
        ...
        if len(set(v for _, v in versions)) > 1:
            inconsistencies.append({"package": pkg, "versions": versions})
...
def main():
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write("## Overview\n\n")
        f.write(
            f"Validated {len(results)} modules for dependency consistency, usage, and version compatibility.\n\n"
        )
        ...
        f.write(
            f"- **Modules with missing dependencies**: {len(modules_with_missing)}/{len(results)}\n"
        )
        f.write(
            f"- **Modules with potentially unused dependencies**: {len(modules_with_unused)}/{len(results)}\n"
        )
        f.write(
            f"- **Modules with dependency file inconsistencies**: {len(modules_with_inconsistencies)}/{len(results)}\n"
        )
        ...
        for result in sorted(results, key=lambda x: x["module"]):
            ...
            f.write(
                f"- **Requirements.txt**: {'✅' if result['has_requirements_txt'] else '❌'}\n"
            )
            f.write(f"- **Setup.py**: {'✅' if result['has_setup_py'] else '❌'}\n")
            f.write(
                f"- **Pyproject.toml**: {'✅' if result['has_pyproject_toml'] else '❌'}\n"
            )
