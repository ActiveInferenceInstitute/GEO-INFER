    ...
    for dep in deps:
        ...
        if dep_name:
            if dep_name not in dep_dict:
                ...
                if ("," in dep_clean or "<" in dep_clean) and (
                    "," not in current and "<" not in current
                ):
                    dep_dict[dep_name] = dep_clean
                elif ("," not in dep_clean and "<" not in dep_clean) and (
...
def cleanup_pyproject(module_path: Path):
    ...
    print(
        f"  ✅ Cleaned {module_path.name}: {len(deps)} -> {len(unique_deps)} dependencies"
    )
