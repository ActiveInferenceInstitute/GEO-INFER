...
class ModuleChecker:
    ...
    def module_exists(self, module_path: str) -> bool:
        ...
        if parts[0].startswith("geo_infer_"):
            ...
            src_path = (
                self.project_root
                / f"GEO-INFER-{module_name.replace('geo_infer_', '').upper()}"
                / "src"
                / module_name
            )
    ...
    for module_path, class_name, line_num in imports:
        ...
        if not module_exists:
            issues["missing_modules"].append(
                {"module": module_path, "class": class_name, "line": line_num}
            )
        else:
...
def main():
    ...
    # Generate report
    report_path = (
        PROJECT_ROOT / "GEO-INFER-INTRA" / "assessment_results" / "AGENTS_DOC_AUDIT.md"
    )
        ...
        for issues in all_issues:
            ...
            if issues["missing_modules"]:
                ...
                for item in issues["missing_modules"]:
                    f.write(
                        f"- Line {item['line']}: `{item['module']}.{item['class']}`\n"
                    )
                f.write("\n")
            ...
            if issues["missing_classes"]:
                ...
                for item in issues["missing_classes"]:
                    f.write(
                        f"- Line {item['line']}: `{item['module']}.{item['class']}` (module exists)\n"
                    )
                f.write("\n")
