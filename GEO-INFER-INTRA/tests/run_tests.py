...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for the GEO-INFER framework"
    )
    ...
    # Module selection
    parser.add_argument(
        "--modules", nargs="*", help="Specific modules to test (e.g., space time data)"
    )
    ...
    parser.add_argument(
        "--marker", "-m", help="Only run tests matching the given marker expression"
    )
    ...
    parser.add_argument(
        "--jobs", "-j", type=int, default=1, help="Number of parallel test processes"
    )
