...
| Preview | Widget |
| --- | --- |
| ![GEO-INFER-ACT Leaflet Preview](previews/geo-infer-act_preview.svg) | Leaflet HTMLMap · SVG vector · PNG raster |

> **Reproducible contract:** each map ships as `geo-infer-act_preview.html`, `geo-infer-act_preview.svg`, `geo-infer-act_preview.png`, and `geo-infer-act_preview.manifest.json` beneath `previews/`. The receipt records an input SHA-256 and artifact accessibility checks.
