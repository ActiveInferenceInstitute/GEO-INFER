...
- **Inference Capabilities**: Generate knowledge based on existing facts
...
### Inference Rules
Rules that derive facts from existing knowledge.
``` mermaid
graph LR
...
```
## Rule Management
GEO-INFER-INTRA provides a system for managing custom rules throughout their lifecycle.
``` mermaid
graph TD
...
[InefficiendRule:
(?feature geo:hasGeometry ?geom) # common pattern, many matches
...
3. **Document Well**: Include comments and documentation for each rule
...
- [Geospatial Analysis](../geospatial/analysis/index.md)
