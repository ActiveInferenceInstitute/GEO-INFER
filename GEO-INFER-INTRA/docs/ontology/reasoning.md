...
IMPLICIT --> KB
subgraph "Example"
A["City(York)"] --> R[Reasoner]
B["Capital(Albany)"] --> R
C["isCapitalOf(Albany, York State)"] --> R
D["contains(York State, York)"] --> R
R --> E["isIn(York, York State)"]
end
...
- **Knowledge Enrichment**: Derive facts from existing knowledge
...
# Reasoning levels
reasoning_level: "FULL" # Options: NONE, RDFS, OWL2_RL,
...
print(f"results count: {len(enhanced_results)}")
...
- [Query Language Reference](../api/reference.md)
- [Performance Tuning Guide](../advanced/performance_optimization.md)
