...
> **Illustrative example notice.** This page contains historical or > conceptual integration sketches. Names such as `SpatialAnalyzer` and > domain-specific facade classes are not public GEO-INFER exports in the > current checkout; verify imports against each module's `src/` package > and use the module README/tests for executable examples. > **Illustrative guide.** The code in this page is illustrative: it sketches > how the module APIs compose for this use case. Some identifiers shown are > conceptual; always import from the current package exports (see the module > `__init__.py` and `SKILL.md`) and prefer the runnable scripts under > `GEO-INFER-*/examples/` for verified behavior. Any numeric results shown > are illustrative and must be reproduced against your own data before use. > **How-to Guide**: Build a environmental monitoring system
>
...
# full workspace uv sync --package geo-infer-act # or just the ACT module uv
...
"""Add sensor data to the monitoring system"""
...
```python
# Create a environmental dashboard
...
"""Generate a environmental summary report"""
report = {
'timestamp': datetime.now(),
...
### Key Findings The workflow demonstrates:
...
3. **Condition Prediction** — horizon predictions from the active inference model.
...
above
are illustrative; run the
module examples on
your own data to obtain measured values.
## 🔧 Features
...
### Related Examples
- **[Urban Air Quality Monitoring](../examples_gallery.md)** - City-scale air quality analysis
- **[Agricultural Monitoring](../examples_gallery.md)** - Crop and soil monitoring
- **[Climate Change Analysis](../examples_gallery.md)** - Long-term climate trends
### Topics
...
- **[Real-time Processing](../advanced/index.md)** - Handle streaming data
---
