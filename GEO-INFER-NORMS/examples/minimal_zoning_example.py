...
class ZoningAnalyzer:
    ...
    def _add_scale_bar(self, ax, gdf):
        ...
        # Add scale bar
        ax.plot([xmin, xmin + scale_bar_length], [ymin, ymin], color='black', linewidth=2)
        ax.text(xmin + scale_bar_length / 2, ymin - 0.01, f'{scale_bar_length:.2f} km', 
                ha='center', va='top', fontsize=8)
    
    def _add_north_arrow(self, ax):
        """Add a north arrow to the plot."""
        # Add north arrow
        ax.annotate('N', xy=(0.95, 0.05), xycoords='axes fraction', ha='right', va='bottom', fontsize=12)
    ...
    def visualize_historical_changes(self, start_date=None, end_date=None, figsize=(12, 8), output_file=None):
        ...
        if changes_by_category:  # Ensure there are changes to plot
            ...
            bars = ax2.barh(range(len(category_names)), category_counts, color=transition_colors)
            ...
            ax2.text(0.5, 0.5, "No category changes available", ha='center', va='center')
        ...
        if district_changes:  # Ensure there are district changes to plot
            ...
            # Create horizontal bar chart
            bars = ax3.barh(range(len(district_names)), district_counts, color=district_colors)
            ...
            ax3.text(0.5, 0.5, "No district changes available", ha='center', va='center')
...
class LandUseClassifier:
    ...
    def classify_land_use(self, parcels_gdf, feature_columns, method="rule_based"):
        """Simple stub implementation that just assigns random land use categories."""
...
def create_sample_data():
    ...
    zoning_codes = [
        ...
        ZoningCode(
            ...
            category="mixed_use",
            jurisdiction_id="city1",
            allowed_uses=["retail", "offices", "apartments", "restaurants", "entertainment"],
            max_height=30,
            min_lot_size=400,
            max_floor_area_ratio=1.2,
            max_density=75,
            setbacks={"front": 2, "rear": 5, "side": 2},
            environmental_requirements={"min_permeable_surface": 0.3, "tree_canopy": 0.15}
        ),
        ZoningCode(
            code="P-1",
            ...
            jurisdiction_id="city1",
            allowed_uses=["parks", "playgrounds", "sports_facilities", "nature_preserves"],
            max_height=12,
            min_lot_size=2000,
        ...
        ZoningCode(
            ...
            environmental_requirements={"min_permeable_surface": 0.9, "tree_canopy": 0.6}
        )
    ]
    ...
    for district in zoning_districts:
        ...
        if district.zoning_code.startswith("R"):
            primary_land_use = "residential_sf" if district.zoning_code == "R-1" else "residential_mf"
        elif district.zoning_code.startswith("C"):
            primary_land_use = "commercial_retail" if district.zoning_code == "C-1" else "commercial_office"
        ...
        for _ in range(num_parcels):
            ...
            for attempt in range(10):  # Try up to 10 times to place a parcel
                ...
                parcel_width = width / math.sqrt(num_parcels) * 0.8  # Scale factor for overlap
                ...
                if district.geometry.contains(parcel_polygon.centroid):
                    ...
                    if random.random() < 0.1:  # 10% chance of different land use
                        land_uses = ["residential_sf", "residential_mf", "commercial_retail", 
                                    "commercial_office", "industrial_light", "parks_recreation"]
                        land_use = random.choice(land_uses)
                    
                    # Building attributes based on land use
                    if land_use.startswith("residential"):
                        building_height = random.uniform(5, 9) if land_use == "residential_sf" else random.uniform(10, 25)
                        building_footprint = parcel_polygon.area * random.uniform(0.2, 0.4)
                        ...
                        building_footprint = parcel_polygon.area * random.uniform(0.4, 0.7)
                        ...
                        building_footprint = parcel_polygon.area * random.uniform(0.5, 0.8)
                        ...
                        building_footprint = parcel_polygon.area * random.uniform(0.05, 0.15)
                        ...
                        building_footprint = parcel_polygon.area * random.uniform(0.4, 0.6)
