...
def main():
    ...
    print(
        f"   ✅ Logging initialized (LOG module available: {log_integration.log_available})"
    )
    ...
    print(
        f"   ✅ Discovered {total_tests} tests across {len(discovered_tests)} modules"
    )
    ...
    with log_integration.test_context("demo_001", "TEST", "log_integration_demo"):
        ...
        test_logger.log_performance_metrics(
            "demo_001",
            {
                "execution_time": 0.5,
    ...
    print(
        f"   ✅ Report generated with {len(log_integration.test_entries)} test entries"
    )
    ...
    if analysis:
        ...
        if analysis["module_reliability"]:
            ...
            for module, metrics in analysis["module_reliability"].items():
                print(
                    f"      {module}: {metrics['success_rate']:.1f}% success, "
    ...
    if bottlenecks:
        ...
        for bottleneck in bottlenecks[:3]:  # Show top 3
            print(
                f"      {bottleneck['test_name']}: {bottleneck['duration']:.3f}s "
