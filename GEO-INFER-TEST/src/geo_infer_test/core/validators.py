...
class SpatialValidator(BaseValidator):
    ...
    def _validate_coordinates(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        lat_valid = (df["latitude"] >= -90) & (df["latitude"] <= 90) & df["latitude"].notna()
        lon_valid = (df["longitude"] >= -180) & (df["longitude"] <= 180) & df["longitude"].notna()
        ...
        return {
            ...
            "coordinate_quality_score": float(both_valid.sum() / len(df)) if len(df) > 0 else 0.0
        }
    ...
    def _analyze_spatial_distribution(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            lats = pd.to_numeric(df["latitude"], errors="coerce").values.astype(np.float64)
            lons = pd.to_numeric(df["longitude"], errors="coerce").values.astype(np.float64)
        except Exception:
...
class IoTValidator(BaseValidator):
    ...
    def _analyze_temporal_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "temporal_coverage": {
                    "start": min(timestamps).isoformat() if not timestamps.empty else "",
                    "end": max(timestamps).isoformat() if not timestamps.empty else "",
                    "span_hours": (max(timestamps) - min(timestamps)).total_seconds() / 3600 if not timestamps.empty else 0
                }
            }
        except Exception as e:
    ...
    def _detect_radiation_anomalies(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        # Ensure numeric types
        try:
            df["radiation_level"] = pd.to_numeric(df["radiation_level"], errors='coerce')
        except Exception:
        ...
        return {
            ...
            "anomaly_rate": float(mild_anomalies / len(radiation_values)) if len(radiation_values) > 0 else 0,
