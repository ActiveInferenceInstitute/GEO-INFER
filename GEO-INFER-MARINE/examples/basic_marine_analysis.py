...
def main():
    ...
    print(
        f"Projected sea level in 2100: {float(projection.sel(time='2100-01-01')):.1f} mm"
    )
    ...
    print(
        f"Bleaching risk by site: {np.round(coral_health['bleaching_risk'].values, 2)}"
    )
    ...
    print(
        f"Shannon diversity: {biodiversity['shannon_diversity']:.3f}, "
