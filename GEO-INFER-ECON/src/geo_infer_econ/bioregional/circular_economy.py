...
class CircularEconomyModels:
    ...
    def model_circular_flows(self, flow_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        logger.info("Modeling circular flows: %d materials, %d processes", len(materials), len(processes))
        ...
        for proc in processes:
            total_input = sum(float(i.get("mass_kg", 0)) for i in proc.get("inputs", []))
            total_output = sum(float(o.get("mass_kg", 0)) for o in proc.get("outputs", []))
        ...
        if not df.empty:
            ...
            logger.info("Average circularity index: %.4f (%s)", avg_mci, self._categorize_circularity(avg_mci))
...
class MaterialFlowAnalysis:
    ...
    def analyze_flows(self, material_data: Dict[str, Any]) -> pd.DataFrame:
        ...
        for f in outflows:
            mat = f.get("material", "unknown")
            outflow_totals[mat] = outflow_totals.get(mat, 0) + float(f.get("mass_kg", 0))
        ...
        all_materials = set(list(inflow_totals.keys()) + list(outflow_totals.keys()) + list(stock_totals.keys()))
        rows = []
        for mat in sorted(all_materials):
            ...
            rows.append({
                ...
                "accumulation_rate": round(balance / max(stock, 1e-6), 4) if stock > 0 else 0,
                "mass_balanced": abs(balance - (stock * 0.01)) < max(inflow * 0.05, 1),
            })
    ...
    def model_industrial_ecology(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for waste in waste_outputs:
            for need in input_needs:
                if waste["material"] == need["material"] and waste["facility_id"] != need["facility_id"]:
                    exchange_mass = min(waste["mass_kg"], need["mass_kg"])
                    savings_estimate = exchange_mass * 0.5  # Assume 50% cost saving vs virgin
        ...
        connected_facilities = len(set(
            [o["supplier"] for o in opportunities] + [o["receiver"] for o in opportunities]
        ))
        ...
        result = {
            ...
            "total_estimated_savings_usd": round(sum(o["estimated_savings_usd"] for o in opportunities), 2),
        }

        logger.info(
            "Found %d symbiosis opportunities, %.1f%% waste diversion",
            len(opportunities), diversion_rate * 100,
        )
        return result
...
class WasteToResourceSystems:
    ...
    DEFAULT_RECOVERY_RATES = {
        "organic": {"composting": 0.85, "anaerobic_digestion": 0.90, "incineration": 0.25},
        "plastic": {"mechanical_recycling": 0.60, "chemical_recycling": 0.75, "incineration": 0.35},
    ...
    def _hierarchy_level(method: str) -> str:
        ...
        recycle_methods = {"recycling", "mechanical_recycling", "chemical_recycling", "composting", "crushing", "sorting"}
...
class RegenerativeDesign:
    ...
    def design_regenerative_system(self, design_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        goals = design_params.get("goals", ["soil_restoration", "biodiversity", "water_cycling"])
        ...
        logger.info("Designing regenerative system: %.1f ha, %d-year timeline", area_ha, timeline)
        ...
        for year in range(timeline + 1):
            ...
            for metric, current_val in current_scores.items():
                ...
                year_scores[metric] = round(current_val + (target_val - current_val) * progress, 4)
            year_scores["year"] = year
            year_scores["composite"] = round(float(np.mean([v for k, v in year_scores.items() if k != "year"])), 4)
