...
class SpaceIntegration:
    ...
    def __init__(self, backend: str = 'h3', config: Optional[Dict[str, Any]] = None):
        ...
        if not SPACE_AVAILABLE:
            logger.warning("SpaceIntegration initialized but GEO-INFER-SPACE not available")
            self.indexer = None
            self.analytics = None
            self.geometry = None
        else:
            try:
                ...
                self.analytics = SpatialAnalyticsInterface(backend='srai' if backend == 'srai' else 'h3')
    ...
    def latlng_to_cell(self, lat: float, lng: float, resolution: int = 9) -> Optional[str]:
        """
