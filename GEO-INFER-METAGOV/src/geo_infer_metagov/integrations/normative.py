...
class NormativeGovernanceIntegration:
    ...
    def __init__(self):
        ...
        if NORMS_AVAILABLE:
            ...
            self.norms_manager = None
            self.norms_available = False
            logger.warning("Normative integration disabled - GEO-INFER-NORMS not available")
    ...
    def translate_governance_rules_to_norms(
        ...
        if governance_rules:
            translation['translation_quality'] = len(translation['normative_rules']) / len(governance_rules)
    ...
    def check_compliance_with_norms(
        ...
        if governance_actions:
            compliance['compliance_rate'] = len(compliance['compliant_actions']) / len(governance_actions)
    ...
    def align_norms_with_governance(
        ...
        for gov_rule in governance_rules:
            gov_rule_type = gov_rule.get('type', '')
        ...
        if existing_norms:
            ...
            alignment['alignment_score'] = max(0.0, 1.0 - conflict_penalty - gap_penalty)
        else:
