...
class TestBoundsToPolygon:
    ...
    def test_bounds_to_polygon_compass_aliases(self) -> None:
        polygon = bounds_to_polygon(
            {"south": 37.0, "north": 38.0, "west": -123.0, "east": -122.0}
        )
        assert polygon == bounds_to_polygon(
            {"min_lat": 37.0, "max_lat": 38.0, "min_lng": -123.0, "max_lng": -122.0}
        )
    ...
    def test_bounds_to_polygon_inverted_bounds_raise(self) -> None:
        with pytest.raises(ValueError, match="min exceeds max"):
            bounds_to_polygon(
                {"min_lat": 38.0, "max_lat": 37.0, "min_lng": -123.0, "max_lng": -122.0}
            )
...
class TestSpatialIntegrationWithSpace:
    ...
    def test_index_point_boundary(
        self, integration: SpatialGovernanceIntegration
    ) -> None:
    ...
    def test_index_bounds_boundary(
        self, integration: SpatialGovernanceIntegration
    ) -> None:
        result = integration.index_governance_boundary(
            {
    ...
    def test_overlapping_boundaries_detected(
        self, integration: SpatialGovernanceIntegration
    ) -> None:
        boundaries = [
            {
    ...
    def test_disjoint_boundaries_no_overlap(
        self, integration: SpatialGovernanceIntegration
    ) -> None:
        boundaries = [
            {
    ...
    def test_coverage_analysis(self, integration: SpatialGovernanceIntegration) -> None:
        ...
        result = integration.analyze_spatial_governance_coverage(
            structure, region, resolution=6
        )
