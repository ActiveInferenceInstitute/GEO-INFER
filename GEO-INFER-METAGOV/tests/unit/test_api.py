...
class TestGovernanceAPICrud:
    ...
    def test_recreate_after_delete_does_not_collide(
        ...
        first = structure_id
        second = api.create_governance_structure({"name": "Second"}, [], []).data[
            "governance_id"
        ]
        assert api.delete_governance_structure(first).code == 200
        third = api.create_governance_structure({"name": "Third"}, [], []).data[
            "governance_id"
        ]
    ...
    def test_analyze_comprehensive_pins_metrics_and_recommendations(
        ...
        assert any(
            "Streamline" in r or "Enhance" in r or "Define" in r
