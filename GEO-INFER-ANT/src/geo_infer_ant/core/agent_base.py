...
class SwarmAgent(BaseAgent if BaseAgent is not object else ABC):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Agent state
        self.energy_level = min(1.0, kwargs.get('initial_energy', 1.0))  # Ensure energy <= 1.0
    ...
    def _initialize_integrations(self, spatial_backend: str) -> None:
        ...
        if self.active_inference_enabled and ActiveInferenceModel:
            try:
                ...
                logger.info(f"Active Inference model initialized for agent {self.agent_id}")
        ...
        if SpatialIndexingInterface:
            try:
                ...
                logger.info(f"Spatial indexer initialized with {spatial_backend} backend")
            except Exception as e:
        ...
        if SpatialAnalyticsInterface:
            try:
                self.spatial_analytics = SpatialAnalyticsInterface(backend=spatial_backend)
                logger.info(f"Spatial analytics initialized with {spatial_backend} backend")
            except Exception as e:
        ...
        if self.active_inference_enabled and self.active_inference_model:
            try:
                ...
                sensory_input.processed_data.update({
                    ...
                    'free_energy': getattr(self.active_inference_model, 'current_free_energy', 0.0)
                })
    ...
    def make_decision(
        ...
        if self.active_inference_enabled and self.active_inference_model:
            try:
                ...
                policies = self.active_inference_model.generate_policies(available_actions)

                # Select optimal action
                optimal_policy = self.active_inference_model.select_policy(policies)
                expected_fe = self.active_inference_model.compute_expected_free_energy(optimal_policy)
                ...
                decision = self._fallback_decision_making(processed_data, internal_motivations, behavioral_rules)
    ...
    def _generate_action_space(self, processed_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if 'social_nearby_agents' in processed_data or processed_data.get('social_nearby_agents', 0) > 0:
            actions.extend([
                {
    ...
    async def _execute_movement_action(self, decision: ActionDecision) -> Dict[str, Any]:
        ...
        self.position = np.clip(new_position, [-180, -90], [180, 90])  # Keep within valid bounds
    ...
    async def _execute_stigmergic_action(self, decision: ActionDecision) -> Dict[str, Any]:
    ...
    async def _execute_communication_action(self, decision: ActionDecision) -> Dict[str, Any]:
    ...
    async def _execute_foraging_action(self, decision: ActionDecision) -> Dict[str, Any]:
    ...
    async def _execute_monitoring_action(self, decision: ActionDecision) -> Dict[str, Any]:
    ...
    def _update_agent_state(self, decision: ActionDecision, result: Dict[str, Any]) -> None:
