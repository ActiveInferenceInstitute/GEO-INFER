...
class ArtificialBeeColony:
    ...
    def optimize(
        ...
        """
        max_iter = max_iterations or self.parameters.max_iterations
        ...
        logger.info(f"ABC optimization completed: best fitness = {self.global_best_fitness}")
    ...
    def _initialize_food_sources(self) -> None:
        ...
        n_sources = self.parameters.colony_size // 2  # Half the colony size
        self.food_sources = []
    ...
    def _employed_bee_phase(self, objective_function: Callable[[np.ndarray], float]) -> None:
    ...
    def _onlooker_bee_phase(self, objective_function: Callable[[np.ndarray], float]) -> None:
    ...
    def _generate_neighbor(self, position: np.ndarray) -> np.ndarray:
        ...
        phi = np.random.uniform(-1, 1)  # Random factor
        new_position = position.copy()
        new_position[dimension] = position[dimension] + phi * (position[dimension] - neighbor_position[dimension])
    ...
    def _evaluate_fitness(self, position: np.ndarray, objective_function: Callable[[np.ndarray], float]) -> float:
    ...
    def manage_food_sources(
        ...
        if abandonment_criteria == 'trial_limit':
            sources_to_abandon = [s for s in current_sources if s.trial_count >= self.parameters.limit]
    ...
    def _apply_fitness_proportional_recruitment(self, sources: List[FoodSource]) -> None:
        """Apply fitness-proportional recruitment strategy."""
        if not sources:
            return
        ...
        for _ in range(n_recruitment):
            # Select source probabilistically
            selected_idx = np.random.choice(len(sources), p=recruitment_probs)
            selected_source = sources[selected_idx]

            # Add recruited bee (simplified)
            # In practice, would create additional search around this source
    ...
    def adapt_foraging_strategy(
        ...
        if behavioral_adaptation == 'learning_automaton':
            ...
            if success_rate < 0.3:
                ...
                adaptation_results['parameters_updated']['max_trials'] = self.parameters.max_trials
            ...
            if 'resource_density' in environmental_conditions:
                ...
                if density < 0.3:
                    ...
                    adaptation_results['parameters_updated']['limit'] = self.parameters.limit
                    adaptation_results['strategy_changes'].append('increased_persistence')

                elif density > 0.8:
                    # Abundant resources - be less persistent
                    self.parameters.limit = max(50, self.parameters.limit * 0.7)
                    adaptation_results['parameters_updated']['limit'] = self.parameters.limit
                    adaptation_results['strategy_changes'].append('decreased_persistence')

        logger.info(f"Foraging strategy adaptation completed: {len(adaptation_results['strategy_changes'])} changes")
    ...
    def get_optimization_statistics(self) -> Dict[str, Any]:
        ...
        if self.food_sources:
            ...
            stats['food_source_statistics'] = {
                ...
                'abandoned_sources': len([s for s in self.food_sources if s.trial_count >= self.parameters.limit])
            }
