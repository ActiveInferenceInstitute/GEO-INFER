...
try:
    ...
    from geo_infer_ant.core.population import AgentPopulation, PopulationConfig, EnvironmentalState
    from geo_infer_ant.core.stigmergy import PheromoneSystem, PheromoneType, PheromoneDeposit, PheromoneField
    from geo_infer_ant.core.digital_stigmergy import DigitalStigmergy, DigitalTrace, InformationQuery
except ImportError:
...
class TestSwarmAgent:
    ...
    def test_agent_sensory_processing(self):
        ...
        sensory_input = SensoryInput(
            spatial_context={'position': agent.position, 'bounds': {'min_lat': -10, 'max_lat': 10}},
    ...
    def test_agent_movement(self):
        ...
        async def move_test():
            ...
            assert np.linalg.norm(agent.position - initial_position) > 0  # Position should change
...
class TestPheromoneSystem:
    ...
    def test_pheromone_deposition(self):
        ...
        async def deposition_test():
            ...
            intensity = pheromone_system.get_pheromone_intensity(np.array([0, 0]), 'trail')
            assert intensity > 0
    ...
    def test_pheromone_diffusion(self):
        ...
        async def diffusion_test():
            ...
            initial_intensity = pheromone_system.get_pheromone_intensity(np.array([0, 0]), 'trail')
            assert initial_intensity > 0

            # Diffuse pheromones
            await pheromone_system.diffuse_pheromones(time_step=60.0)

            final_intensity = pheromone_system.get_pheromone_intensity(np.array([0, 0]), 'trail')
...
class TestDigitalStigmergy:
    ...
    def test_information_query(self):
        ...
        async def query_test():
            ...
            assert all(trace.information_type in ['sensor_data', 'alerts'] for trace in results)
    ...
    def test_pattern_extraction(self):
        ...
        async def pattern_test():
            ...
            for i in range(10):
                await digital_stigmergy.contribute_information(
                    agent_id=f"agent_{i}",
                    information_type=np.random.choice(['sensor_data', 'alerts', 'coordination']),
