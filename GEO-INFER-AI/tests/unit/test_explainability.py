...
class TestModelExplainer:
    ...
    def test_calculate_feature_importance_permutation(
        ...
        importances = explainer.calculate_feature_importance(
            sample_data, y, method="permutation"
        )
    ...
    def test_spatial_explanation(
        self, trained_regressor: RandomForestRegressor
    ) -> None:
        np.random.seed(42)
        ...
        explainer = ModelExplainer(trained_regressor)
        result = explainer.generate_spatial_explanation(
            spatial_features, predictions, coordinates
        )
