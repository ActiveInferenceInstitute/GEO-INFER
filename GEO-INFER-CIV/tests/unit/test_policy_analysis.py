...
class TestCostBenefitAnalyzer:
    ...
    def test_simple_analysis(self, cba):
        cba.add_item(
            CostBenefitItem(
                "construction", 1_000_000, is_benefit=False, time_horizon_years=0
            )
        )
        cba.add_item(
    ...
    def test_negative_npv(self, cba):
        ...
        cba.add_item(
            CostBenefitItem("benefit", 100_000, is_benefit=True, time_horizon_years=1)
        )
        result = cba.analyze()
        assert result.net_present_value < 0
        assert result.benefit_cost_ratio < 1.0
    ...
    def test_risk_adjusted(self, cba):
        cba.add_item(
            CostBenefitItem("cost", 100_000, is_benefit=False, probability=1.0)
        )
        cba.add_item(
            CostBenefitItem("benefit_certain", 80_000, is_benefit=True, probability=1.0)
        )
        cba.add_item(
            CostBenefitItem("benefit_risky", 80_000, is_benefit=True, probability=0.5)
        )
        result = cba.analyze()
    ...
    def test_payback_period_inf_when_never_recovered(self, cba):
        ...
        cba.add_item(
            CostBenefitItem("income", 100, is_benefit=True, time_horizon_years=1)
        )
        result = cba.analyze()
    ...
    def test_irr_is_nan_without_sign_crossing(self, cba):
        ...
        cba.add_item(
            CostBenefitItem("cost", 500, is_benefit=False, time_horizon_years=1)
        )
        result = cba.analyze()
    ...
    def test_irr_lands_between_bisection_bounds(self, cba):
        ...
        cba.add_item(
            CostBenefitItem("income", 600, is_benefit=True, time_horizon_years=1)
        )
        result = cba.analyze()
    ...
    def test_impact_matrix(self, stakeholder_analyzer):
        ...
        stakeholder_analyzer.add_impact(
            StakeholderImpact(
                ...
                economic_impact=1.0,
                quality_of_life_impact=0.2,
                environmental_impact=-0.3,
                accessibility_impact=0.5,
            )
        )
