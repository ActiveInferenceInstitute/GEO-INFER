...
from geo_infer_pep.crm.transformer import (
    clean_customer_data,
    enrich_customer_data,
    convert_customers_to_dataframe,
)
from geo_infer_pep.reporting.crm_reports import (
    generate_customer_segmentation_report,
    generate_lead_conversion_report,
)
from geo_infer_pep.visualizations.crm_visuals import (
...
def test_plot_customer_distribution_by_status(sample_customer_data_list, tmp_path):
    ...
    output_dir.mkdir()
    plot_path = plot_customer_distribution_by_status(
        sample_customer_data_list, output_dir=output_dir
    )
...
def test_plot_customer_distribution_by_source(sample_customer_data_list, tmp_path):
    ...
    plot_path = plot_customer_distribution_by_source(
        sample_customer_data_list, output_dir=output_dir
    )
    assert plot_path is not None
    assert os.path.exists(plot_path)
...
def test_crm_visuals_empty_data(tmp_path):
    ...
    output_dir.mkdir()
    status_plot = plot_customer_distribution_by_status(
        empty_list, output_dir=output_dir
    )
    assert status_plot is None
    source_plot = plot_customer_distribution_by_source(
        empty_list, output_dir=output_dir
    )
...
def test_csv_crm_importer_file_not_found():
    ...
    with pytest.raises(
        FileNotFoundError
    ):  # Assuming connect is called within import_customers
