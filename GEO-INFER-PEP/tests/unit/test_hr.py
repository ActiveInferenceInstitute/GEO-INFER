...
from geo_infer_pep.hr.transformer import (
    clean_employee_data,
...
from geo_infer_pep.visualizations.hr_visuals import (
...
def test_generate_headcount_report(sample_employee_data_list):
    ...
    report = generate_headcount_report(
        sample_employee_data_list, group_by=["department"]
    )
...
def test_generate_diversity_report(sample_employee_data_list):
    ...
    report = generate_diversity_report(
        sample_employee_data_list, diversity_fields=["gender", "department"]
    )
...
def test_plot_headcount_by_department(sample_employee_data_list, tmp_path):
    ...
    output_dir.mkdir()
    plot_path = plot_headcount_by_department(
        sample_employee_data_list, output_dir=output_dir
    )
...
def test_plot_gender_distribution(sample_employee_data_list, tmp_path):
    ...
    plot_path = plot_gender_distribution(
        sample_employee_data_list, output_dir=output_dir
    )
    assert plot_path is not None
    assert os.path.exists(plot_path)
