...
class TestWildfireRiskAnalyzer:
    ...
    def test_assess_wildfire_risk(self, analyzer):
        ...
        precip = xr.DataArray([[10, 5, 2], [8, 4, 1]], dims=["time", "location"])
...
class TestSuppressionPlanning:
    ...
    def test_suppression_terrain_factor(self, analyzer):
        ...
        assert (
            difficult["personnel"]["firefighters_needed"]
            > easy["personnel"]["firefighters_needed"]
        )
...
class TestPostFireDamage:
    ...
    def test_assess_post_fire_damage(self, analyzer):
        ...
        pre_ndvi = xr.DataArray([[0.7, 0.8, 0.75], [0.65, 0.7, 0.8]], dims=["y", "x"])
        post_ndvi = xr.DataArray([[0.2, 0.3, 0.6], [0.1, 0.5, 0.75]], dims=["y", "x"])
