...
def main():
    ...
    print(f"   {'Station':<12} {'Temp':>8} {'RH':>6} {'Wind':>8} {'FWI':>8} {'Rating':>12}")
    print(f"   {'-'*56}")
    ...
    for obs in weather_stations:
        ...
        print(f"   {obs.observation_id:<12} {obs.temperature_c:>6.0f}°C {obs.relative_humidity:>5.0f}% "
    ...
    for scenario in scenarios:
        ...
        print(f"   {scenario['fuel'].value:<22} {scenario['wind']:>6} km/h {scenario['slope']:>6}% "
    ...
    print(f"   Backing fire rate: {critical['spread_rates']['back_m_per_min']:.1f} m/min")
    ...
    for size in fire_sizes:
        ...
        print(f"   Personnel: {resources['personnel']['firefighters_needed']} firefighters "
              f"({resources['personnel']['crews_20_person']} crews)")
        print(f"   Equipment: {resources['equipment']['engines']} engines, "
              f"{resources['equipment']['helicopters']} helicopters, "
              f"{resources['equipment']['airtankers']} airtankers")
        print(f"   Est. containment: {resources['timeline']['estimated_containment_days']} days")
    ...
    pre_ndvi = xr.DataArray(
        0.6 + np.random.uniform(0, 0.3, (20, 20)),
        dims=['y', 'x']
    )
    ...
    print(f"   Burn Severity Analysis:")
    ...
    for incident in analyzer.get_active_incidents():
        print(f"   {incident.incident_id:<15} {incident.name:<20} "
