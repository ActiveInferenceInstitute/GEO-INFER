# GEO-INFER-AGENT/tests

Tests workspace within `GEO-INFER-AGENT`.
...
- Purpose: validate the `GEO-INFER-AGENT` module's current behavior through unit,
...
- Failure triage: `env -u VIRTUAL_ENV uv run pytest -c pyproject.toml -q
  GEO-INFER-AGENT/tests`, followed by
