...
class TestConfigurationToInterface:
    ...
    def test_default_config_drives_agent_creation(self) -> None:
        ...
        agent_id = interface.create_agent(
            AgentType.BDI,
            {
                "name": "IntegrationBot",
...
class TestFactoryInterfaceLifecycle:
    ...
    def test_full_bdi_lifecycle(self) -> None:
        ...
        state = interface.get_agent_state(agent_id)
        assert (
            "explore" in state.metadata["intentions"]
            or "avoid_obstacle" in state.metadata["intentions"]
        )
...
class TestHaversineDistance:
    ...
    def test_none_inputs_return_false(self) -> None:
        interface = BDIAgentInterface()
        assert (
            interface._is_location_in_radius(None, {"lat": 0.0, "lng": 0.0}, 10.0)
            is False
        )
        assert (
            interface._is_location_in_radius({"lat": 0.0, "lng": 0.0}, None, 10.0)
            is False
        )
...
class TestAsyncAPIPipeline:
    ...
    async def test_complete_agent_lifecycle(self) -> None:
        client = AgentAPIClient(
            config={"agents_config_path": "/tmp/test_pipeline_agents.json"}
        )
    ...
    async def test_update_command_persists(self) -> None:
        client = AgentAPIClient(
            config={"agents_config_path": "/tmp/test_pipeline_update.json"}
        )
