...
class MetricsCollector:
    ...
    def _start_auto_export(self) -> None:
        ...
        export_thread = threading.Thread(target=self._auto_export_worker, daemon=True)
        export_thread.start()
    ...
    def counter(
        ...
        with self.lock:
            ...
            # Store metric
            metric = Metric(name=name, value=self.counters[name], tags=tags or {})
...
class Tracer:
    ...
    def start_span(
        ...
        if not self._should_sample():
            # Create a no-op span
            return TraceSpan(
                span_id=span_id, trace_id=trace_id, operation_name=operation_name
            )
    ...
    def _export_traces_json(self) -> str:
        ...
        export_data: Dict[str, Any] = {"service": self.service_name, "traces": {}}
...
class HealthChecker:
    ...
    def get_status(self) -> str:
        ...
        statuses = [
            check.get("status", "unknown") for check in self.last_check_results.values()
        ]
...
class AlertManager:
    ...
    def __init__(self) -> None:
        ...
        self.evaluation_thread = threading.Thread(
            target=self._alert_evaluation_worker, daemon=True
        )
    ...
    def evaluate_metrics(self, metrics_collector: MetricsCollector) -> None:
        ...
        for rule in self.rules.values():
            ...
            if suppression_key in self.suppression_rules:
                if current_time - self.suppression_rules[suppression_key] < timedelta(
            ...
            if triggered:
                ...
                alert = Alert(
                    ...
                    or f"Alert: {rule.metric_name} {rule.condition} {rule.threshold}",
    ...
    def _get_metric_value(
        self, metric_name: str, metrics_collector: MetricsCollector
    ) -> Optional[float]:
...
class ObservabilityManager:
    ...
    def __init__(self, service_name: str = "geo-infer-git"):
        ...
        self.alert_thread = threading.Thread(
            target=self._auto_alert_worker, daemon=True
        )
...
def create_observability_manager(
    service_name: str = "geo-infer-git",
