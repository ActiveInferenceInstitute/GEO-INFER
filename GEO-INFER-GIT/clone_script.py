...
OUTPUT_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "cloned_repositories"
)
...
def main():
        ...
        for user_info in users:
            ...
            try:
                ...
                for repo in user_repos:
                    ...
                    if exclude_repos:
                        if any(
                            match_wildcard(repo_name, pattern)
                ...
                logger.info(
                    f"Found {len(filtered_repos)} repositories for user {username}"
                )
                ...
                with ThreadPoolExecutor(max_workers=4) as executor:
                    ...
                    for repo in filtered_repos:
                        repo_name = repo.get("name")
                        futures.append(
                            executor.submit(clone_repository, username, repo_name)
                        )
                        total_repos += 1
    ...
    # Print summary
    logger.info(
        f"Cloning complete: {success_repos}/{total_repos} repositories cloned successfully"
    )
