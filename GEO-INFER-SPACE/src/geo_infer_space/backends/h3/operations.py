...
# This legacy module retains two compatible operation families (set-oriented
# and list-oriented) for downstream callers; their overlapping names are
# intentional and the later definitions provide the public list-oriented API.
# ruff: noqa: F811
...
def cell_to_boundary(
    h3_index: str, geo_json_format: bool = False
) -> Union[List[Tuple[float, float]], List[List[float]]]:
    """
...
def grid_distance(h3_index1: str, h3_index2: str) -> int:
    ...
    try:
        ...
        logger.error(
            f"Failed to calculate distance between {h3_index1} and {h3_index2}: {e}"
        )
        raise
...
def cell_to_parent(h3_index: str, parent_resolution: int) -> str:
    ...
    if parent_resolution >= current_resolution:
        raise ValueError(
            f"Parent resolution {parent_resolution} must be < current resolution {current_resolution}"
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to get parent of {h3_index} at resolution {parent_resolution}: {e}"
        )
        raise
...
def cell_to_children(h3_index: str, child_resolution: int) -> List[str]:
    ...
    if child_resolution <= current_resolution:
        raise ValueError(
            f"Child resolution {child_resolution} must be > current resolution {current_resolution}"
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to get children of {h3_index} at resolution {child_resolution}: {e}"
        )
        raise
...
def compact_cells(h3_indices: List[str]) -> List[str]:
    ...
    if invalid_indices:
        raise ValueError(
            f"Invalid H3 cell indices: {invalid_indices[:5]}..."
        )  # Show first 5
...
def uncompact_cells(h3_indices: List[str], target_resolution: int) -> List[str]:
    ...
    if not 0 <= target_resolution <= 15:
        raise ValueError(
            f"Target resolution {target_resolution} must be between 0 and 15"
        )
    ...
    try:
        ...
        logger.error(
            f"Failed to uncompact {len(h3_indices)} cells to resolution {target_resolution}: {e}"
        )
        raise
...
def polygon_to_cells(
    polygon_coords: Union[List[Tuple[float, float]], Dict[str, Any]],
    resolution: int,
    geo_json_format: bool = False,
) -> List[str]:
    """
