...
def fix_h3_v3_api_calls(file_path: str) -> Tuple[bool, List[str]]:
    ...
    # Additional common v3->v4 replacements
    simple_replacements = {
        r'\bh3\.geo_to_h3\s*\(': 'h3.latlng_to_cell(',
        r'\bh3\.h3_to_geo\s*\(': 'h3.cell_to_latlng(',
        r'\bh3\.h3_to_geo_boundary\s*\(': 'h3.cell_to_boundary(',
        r'\bh3\.k_ring\s*\(': 'h3.grid_disk(',
        r'\bh3\.hex_ring\s*\(': 'h3.grid_ring(',
        r'\bh3\.h3_distance\s*\(': 'h3.grid_distance(',
        r'\bh3\.compact\s*\(': 'h3.compact_cells(',
        r'\bh3\.uncompact\s*\(': 'h3.uncompact_cells(',
        r'\bh3\.polyfill\s*\(': 'h3.polygon_to_cells(',
    }
    for patt, repl in simple_replacements.items():
        if re.search(patt, content):
            content = re.sub(patt, repl, content)
            changes.append(f"Applied replacement: {patt} -> {repl}")
