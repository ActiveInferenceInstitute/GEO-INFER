...
This script will:
1. Update all requirements.txt files to use h3>=4.5.0,<5
...
class H3V4FrameworkUpgrader:
    ...
    def upgrade_requirements_files(self) -> None:
        ...
        for req_file in self.framework_root.rglob("requirements*.txt"):
            try:
                ...
                # Update h3>=3.x.x to h3>=4.5.0,<5
                content = re.sub(r"h3>=3\.\d+\.\d+", "h3>=4.5.0,<5", content)
    ...
    def upgrade_documentation(self) -> None:
        ...
        for doc_file in self.framework_root.rglob("*.md"):
            try:
                ...
                content = re.sub(r"h3\s*>=?\s*3\.\d+", "h3>=4.5.0,<5", content)
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            "2. Update virtual environments with H3 v4.5: `uv pip install 'h3>=4.5.0,<5'`"
        )
...
def main():
    ...
    )  # Go up from tests/GEO-INFER-SPACE to GEO-INFER

    if not framework_root.exists():
        framework_root = (
            current_dir.parent
        )  # Fallback to GEO-INFER-SPACE if not in full framework
