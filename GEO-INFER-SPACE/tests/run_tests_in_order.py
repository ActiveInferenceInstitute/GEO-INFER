...
def run_test_category(category, verbose=False):
    ...
    try:
        result = subprocess.run(
            cmd, cwd=Path(__file__).parent, check=False, timeout=600
        )  # 10 minutes
...
def run_all_tests(verbose=False):
    ...
    try:
        result = subprocess.run(
            cmd, cwd=Path(__file__).parent, check=False, timeout=900
        )  # 15 minutes
        return result.returncode == 0
    except subprocess.TimeoutExpired:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run GEO-INFER-SPACE tests in logical order"
    )
    parser.add_argument(
        "--category",
        choices=["setup", "core", "spatial", "reporting", "all"],
        default="all",
