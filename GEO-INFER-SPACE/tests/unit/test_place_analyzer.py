...
class TestPlaceAnalyzer(unittest.TestCase):
    ...
    def test_load_place_data(self):
        ...
        self.assertSetEqual(
            set(self.analyzer.integrated_data["source"]), {"test", "test2", "test3"}
        )
    ...
    def test_perform_spatial_analysis(self):
        ...
        self.assertIsInstance(
            self.analyzer.analysis_results["buffer"], gpd.GeoDataFrame
        )
        # Overlay result should be a GeoDataFrame and not empty
        self.assertIsInstance(
            self.analyzer.analysis_results["overlay"], gpd.GeoDataFrame
        )
