...
def test_spatial_filter(spatial_methods):
    ...
    res = spatial_methods.spatial_filter(
        ["a", "b", "c"], [10, 20, 30], filter_type="threshold", threshold=15
    )
...
def test_aggregate_to_region(spatial_methods):
    ...
    res = spatial_methods.aggregate_to_region(
        ["cell_1", "cell_2"], [10.0, 20.0], target_resolution=8, aggregation="mean"
    )
...
def test_disaggregate_to_cells(spatial_methods):
    ...
    res = spatial_methods.disaggregate_to_cells(
        ["parent1"], [100.0], target_resolution=10, method="equal"
    )
    assert "output_cells" in res
...
def test_find_spatial_outliers(spatial_methods):
    ...
    res = spatial_methods.find_spatial_outliers(
        ["cell1", "neighbor_cell1"], [100.0, 10.0]
    )
...
def test_calculate_spatial_weights(spatial_methods):
    ...
    res = spatial_methods.calculate_spatial_weights(
        ["cell1", "neighbor_cell1"], weight_type="queen"
    )
