...
class NestedH3Orchestrator:
    ...
    def scenario_1_urban_planning(self) -> Dict[str, Any]:
        ...
        cells = []
        for i in range(50):
            lat = center_lat + (np.random.uniform(-0.05, 0.05) if NUMPY_AVAILABLE else (i % 10 - 5) * 0.01)
            lng = center_lng + (np.random.uniform(-0.05, 0.05) if NUMPY_AVAILABLE else (i // 10 - 5) * 0.01)
            ...
            urban_data[cell_index] = {
                ...
                'density': density,
                'type': d_type,
                'infrastructure': np.random.uniform(0.3, 1.0) if NUMPY_AVAILABLE else 0.7,
                'lat': lat, 
                'lng': lng
            }
        ...
        for c, data in urban_data.items():
            t = data['type']
            if t not in zones: zones[t] = []
            zones[t].append(c)
            
        print(f"✅ Grouped cells into {len(zones)} zones based on type")
        scenario_results['data_outputs']['zoning'] = {k: len(v) for k, v in zones.items()}
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            try:
                ...
                sc = axes[0].scatter(lngs, lats, c=pops, cmap='YlOrRd', s=100, alpha=0.7)
                ...
                sc2 = axes[1].scatter(lngs, lats, c=colors, cmap='tab10', s=100, alpha=0.7)
                axes[1].set_title('Zoning Types')
                
                viz_filename = f"urban_planning_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                plt.tight_layout()
    ...
    def scenario_2_environmental_monitoring(self) -> Dict[str, Any]:
        ...
        avg_temp = sum(d['temperature'] for d in sensor_data.values()) / len(sensor_data)
        max_aqi = max(d['aqi'] for d in sensor_data.values())
        
        print(f"📊 Network Stats: Avg Temp {avg_temp:.1f}°C, Max AQI {max_aqi}")
        scenario_results['data_outputs']['stats'] = {'avg_temp': avg_temp, 'max_aqi': max_aqi}
    ...
    def scenario_3_supply_chain(self) -> Dict[str, Any]:
        ...
        for c, data in nodes.items():
            if data['type'] == 'retail':
                # Find nearest warehouse (simple mock)
                flows.append({
                    'from': 'nearest_warehouse',
                    'to': c,
                    'amount': data['demand']
                })
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            for c, d in nodes.items():
                ...
                ax.scatter(d['lng'], d['lat'], c=color, marker=marker, s=100, label=d['type'])
