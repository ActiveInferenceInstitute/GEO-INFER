...
class ExtremeEventAnalyzer:
    ...
    def calculate_return_period(
        ...
        return {
            'value': float(value),
            'method': method,
            'return_period_years': float(estimated_rp) if estimated_rp != float('inf') else None,
    ...
    def detect_compound_events(
        ...
        precip_threshold = float(precipitation.quantile(precip_threshold_percentile / 100.0))
    ...
    def calculate_climate_indices(
        ...
        if precipitation is not None:
            ...
            indices['SDII'] = float(np.mean(precip_values[precip_values >= 1]))  # Simple intensity
    ...
    def _max_consecutive_dry_days(self, precip: np.ndarray, threshold: float = 1.0) -> int:
    ...
    def _max_consecutive_wet_days(self, precip: np.ndarray, threshold: float = 1.0) -> int:
    ...
    def get_event_statistics(self) -> Dict[str, Any]:
        ...
        for event in self.event_registry:
            ...
            by_type[etype]['severities'][sev] = by_type[etype]['severities'].get(sev, 0) + 1
        
        # Calculate averages
        for etype in by_type:
            by_type[etype]['avg_duration'] = by_type[etype]['total_duration'] / by_type[etype]['count']
