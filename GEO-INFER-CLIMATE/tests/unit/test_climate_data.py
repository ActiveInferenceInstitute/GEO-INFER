...
def test_extract_temporal_subset():
    ...
    data = xr.Dataset(
        {"temperature": (["time"], np.random.randn(100))}, coords={"time": time}
    )

    subset = processor.extract_temporal_subset(data, "2000-01-10", "2000-01-20")
...
def test_extract_spatial_subset():
    ...
    subset = processor.extract_spatial_subset(data, (-45, 45), (-90, 90))
