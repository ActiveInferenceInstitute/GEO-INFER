...
def create_sample_climate_data():
    ...
    temperature = (
        20
        + 10 * np.sin(2 * np.pi * np.arange(365) / 365)[:, None, None]
        + np.random.randn(365, 10, 10) * 2
    )
    precipitation = np.maximum(
        0,
        2
        + np.sin(2 * np.pi * np.arange(365) / 365)[:, None, None]
        + np.random.randn(365, 10, 10) * 0.5,
    )
...
def main():
    ...
    print(
        f"   SPI calculated: mean={spi.mean().values:.2f}, std={spi.std().values:.2f}"
    )
