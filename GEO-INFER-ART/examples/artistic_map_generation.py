...
def create_sample_geo_data():
    ...
    data = {"region": ["A", "B", "C", "D"], "value": [10, 20, 30, 40]}
...
def example_2_color_palettes():
    ...
    theory_palette = ColorPalette.from_color_theory(
        base_color="#1E88E5", scheme="analogous", n_colors=6
    )
...
def example_4_generative_maps():
    ...
    for style in styles:
        ...
        gen_map = GenerativeMap.from_elevation(
            region=terrain, resolution=512, abstraction_level=0.5, style=style
        )
    ...
    for level in [0.1, 0.5, 0.9]:
        gen_map = GenerativeMap.from_elevation(
            region=terrain, resolution=512, abstraction_level=level, style="contour"
        )
...
def example_5_procedural_art():
    ...
    for lat, lon, name in coordinates:
        ...
        proc_art = ProceduralArt.from_geo_coordinates(
            lat=lat, lon=lon, resolution=(600, 600)
        )
    ...
    for algorithm in algorithms:
        try:
            ...
            proc_art = ProceduralArt.from_geo_coordinates(
                lat=lat, lon=lon, algorithm=algorithm, resolution=(600, 600)
            )
    ...
    for i, (lat, lon, name, feature) in enumerate(places):
        ...
        try:
            ...
            if feature is not None:
                place_art = PlaceArt.from_place_name(place_name=feature, style=style)
            else:
                place_art = PlaceArt.from_coordinates(
                    lat=lat, lon=lon, name=name, style=style, radius_km=5.0
                )
...
def example_7_cultural_maps():
    ...
    for i, (region, theme) in enumerate(regions):
        ...
        try:
            ...
            cultural_map = CulturalMap.from_region(
                region_name=region, cultural_theme=theme, style=style
            )
    ...
    try:
        ...
        # Add narrative
        narrative = (
            "This map depicts the historical context of Rome and its surroundings."
        )
        cultural_map.add_narrative(narrative=narrative, position="bottom")
