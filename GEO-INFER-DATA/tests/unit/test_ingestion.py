...
class TestMultiSourceDataIngestion:
    ...
    def test_unsupported_data_source(self, ingestion_system, mock_satellite_data):
        ...
        with pytest.raises(ValueError, match="not supported"):
            asyncio.run(ingestion_system.ingest_multi_source(
                satellite=mock_satellite_data,
                unsupported_source={'data': 'test'}
            ))
    ...
    async def test_single_source_ingestion(self, ingestion_system, mock_satellite_data):
        ...
        connector.validate_data = AsyncMock(return_value=QualityCheck(
            score=0.9,
            status=QualityStatus.PASS,
            issues=[]
        ))

        result = await ingestion_system.ingest_multi_source(satellite=mock_satellite_data)
    ...
    @pytest.mark.asyncio
    async def test_multi_source_ingestion(self, ingestion_system, mock_satellite_data, mock_sensor_data):
        ...
        for source_name in ['satellite', 'sensors']:
            ...
            connector.connect = AsyncMock(return_value=True)
            connector.fetch_data = AsyncMock(return_value={'data': f'mock_{source_name}_data'})
            connector.validate_data = AsyncMock(return_value=QualityCheck(
                score=0.9,
                status=QualityStatus.PASS,
                issues=[]
    ...
    def test_quality_report_generation(self, ingestion_system):
        ...
        ingested_data = {
            ...
            'quality_reports': {
                'satellite': QualityCheck(score=0.9, status=QualityStatus.PASS, issues=[])
            }
        }
    ...
    def test_accuracy_calculation(self, ingestion_system):
        ...
        clean_data = {
            'data': pd.DataFrame({
                'temperature': np.random.normal(20, 1, 1000),  # Low variance = high accuracy
