...
class TestEndToEndWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_complete_ingestion_to_storage_workflow(self, mock_environmental_data, mock_metadata):
    ...
    async def test_multi_source_integration(self):
        ...
        print(f"   - Sources processed: {result['ingestion_metadata']['sources_processed']}")
        print(f"   - Parallel processing: {result['ingestion_metadata']['parallel_processing']}")

    @pytest.mark.asyncio
    async def test_storage_backend_integration(self, mock_environmental_data, mock_metadata):
        ...
        realtime_results = await storage.adaptive_query(
    ...
    @pytest.mark.asyncio
    async def test_quality_validation_integration(self, mock_environmental_data, mock_metadata):
        ...
        completeness_check = await quality_manager.validator.validate_data(mock_environmental_data)
        assert completeness_check.score >= 0.0

        # Test coordinate validation
        coord_check = quality_manager.validator.validate_coordinates(mock_environmental_data)
        assert coord_check.score >= 0.0

        # Test temporal validation
        temporal_check = quality_manager.validator.validate_temporal_data(mock_environmental_data)
    ...
    async def test_error_recovery_integration(self):
        ...
        try:
            await initialize_data_system(
                storage_backends=['invalid_backend'],
                enable_validation=True
            )
    ...
    @pytest.mark.asyncio
    async def test_performance_integration(self, mock_environmental_data, mock_metadata):
        ...
        ingestion_result = await system['ingestion'].ingest_multi_source(
        ...
        query_results = await system['storage'].adaptive_query(
            spatial_bounds=[-122.5, 37.7, -122.3, 37.9]
        )
...
class TestCrossComponentIntegration:
    ...
    async def test_data_flow_ingestion_to_storage(self):
        ...
        metadata = DatasetMetadata(
            ...
            temporal=TemporalExtent(start=datetime(2023, 1, 1), end=datetime(2023, 1, 5)),
            lineage=DataLineage(source="test", process="integration_test", created_by="test")
        )
    ...
    async def test_quality_to_storage_integration(self):
        ...
        data_with_issues = pd.DataFrame({
            ...
            'latitude': [200 if i < 5 else 37.7 for i in range(100)],   # Invalid coords
            'longitude': [-300 if i < 5 else -122.4 for i in range(100)] # Invalid coords
        })
        ...
        metadata = DatasetMetadata(
            ...
            lineage=DataLineage(source="test", process="quality_test", created_by="test")
        )
        ...
        dataset_id = await storage.store_geospatial_data(
            data_with_issues,
            metadata,
            access_patterns={'query_frequency': 'low'}
        )
...
class TestDataFlowIntegration:
    ...
    async def test_geospatial_data_pipeline(self):
        ...
        gdf = gpd.GeoDataFrame({
            ...
            np.random.normal(-122.4, 0.1, 100),
            np.random.normal(37.7, 0.1, 100)
        ), crs="EPSG:4326")
        ...
        metadata = DatasetMetadata(
            ...
            spatial=SpatialExtent(bbox=[-122.5, 37.6, -122.3, 37.8]),
            temporal=TemporalExtent(start=datetime(2023, 1, 1), end=datetime(2023, 1, 2)),
            lineage=DataLineage(source="test", process="geospatial_test", created_by="test")
        )
    ...
    async def test_temporal_data_pipeline(self):
        ...
        metadata = DatasetMetadata(
            ...
            lineage=DataLineage(source="test", process="temporal_test", created_by="test")
        )
...
class TestPerformanceIntegration:
    ...
    async def test_large_dataset_performance(self):
        ...
        metadata = DatasetMetadata(
            ...
            spatial=SpatialExtent(bbox=[-122.5, 37.6, -122.3, 37.8]),
            temporal=TemporalExtent(start=datetime(2023, 1, 1), end=datetime(2023, 1, 2)),
            lineage=DataLineage(source="performance_test", process="large_data", created_by="test")
        )
        ...
        # Test query performance
        start_time = time.time()
        results = await storage.adaptive_query(spatial_bounds=[-122.5, 37.6, -122.3, 37.8])
        query_time = time.time() - start_time
    ...
    async def test_concurrent_operations(self):
        ...
        async def concurrent_storage(dataset_id: str, data: pd.DataFrame, metadata: DatasetMetadata):
        ...
        metadata = DatasetMetadata(
            ...
            spatial=SpatialExtent(bbox=[-122.5, 37.6, -122.3, 37.8]),
            temporal=TemporalExtent(start=datetime(2023, 1, 1), end=datetime(2023, 1, 2)),
            lineage=DataLineage(source="concurrent_test", process="parallel", created_by="test")
        )
