...
class CoordinateReferenceSystem(BaseModel):
    ...
    wkt: Optional[str] = Field(default=None, description="Well-known text representation")
...
class SpatialExtent(BaseModel):
    ...
    geometry_wkt: Optional[str] = Field(default=None, description="Geometry in WKT format")
    ...
    def validate_crs(cls, v):
        ...
        if isinstance(v, str):
            ...
            raise ValueError("CRS must be a string, dictionary, or CoordinateReferenceSystem instance")
...
class TemporalExtent(BaseModel):
    ...
    resolution: Optional[str] = Field(
        default=None,
        description="Temporal resolution (ISO 8601 duration)"
    )
...
class DatasetMetadata(BaseModel):
    ...
    spatial: SpatialExtent = Field(..., description="Spatial extent and CRS")
    temporal: Optional[TemporalExtent] = Field(default=None, description="Temporal extent and resolution")
    lineage: DataLineage = Field(..., description="Data provenance information")
    quality: Dict[str, QualityCheck] = Field(
        default_factory=dict,
        description="Quality check results by category"
    )
...
class Dataset(BaseModel):
    ...
    storage_backend: StorageBackend = Field(
        default=StorageBackend.POSTGRESQL,
...
class DataDestination(BaseModel):
    ...
    type: str = Field(..., description="Destination type (dataset, database, file, api)")
    configuration: Dict[str, Any] = Field(..., description="Destination-specific configuration")
...
class ETLPipeline(BaseModel):
    ...
    destination: DataDestination = Field(..., description="Data destination configuration")
