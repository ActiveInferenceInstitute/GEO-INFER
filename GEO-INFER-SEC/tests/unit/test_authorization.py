...
class TestAuthorizationManager:
    ...
    def test_grant_and_check_permission(self, auth_manager):
        ...
        assert (
            auth_manager.check_permission("user-1", "dataset-a", PermissionType.READ)
            is True
        )

    def test_check_permission_denied(self, auth_manager):
        assert (
            auth_manager.check_permission("user-1", "dataset-a", PermissionType.WRITE)
            is False
        )
    ...
    def test_revoke_permission(self, auth_manager):
        ...
        assert (
            auth_manager.check_permission("user-1", "resource-x", PermissionType.WRITE)
            is True
        )
    ...
    def test_admin_permission_grants_all(self, auth_manager):
        ...
        assert (
            auth_manager.check_permission(
                "admin-user", "any-resource", PermissionType.DELETE
            )
            is True
        )
    ...
    def test_wildcard_resource_permission(self, auth_manager):
        ...
        assert (
            auth_manager.check_permission("ops-user", "monitoring", PermissionType.READ)
            is True
        )
