...
class TestGeospatialAnonymizer(unittest.TestCase):
        ...
        self.points_gdf = gpd.GeoDataFrame(
            {
                ...
                "geometry": [
                    Point(lon, lat) for lon, lat in zip(longitudes, latitudes)
                ],
            },
            crs="EPSG:4326",
        )
    ...
    def test_location_perturbation(self):
        ...
        # Apply perturbation
        perturbed = self.anonymizer.location_perturbation(
            self.points_gdf, epsilon=500.0
        )
    ...
    def test_spatial_k_anonymity(self):
        ...
        anonymized = self.anonymizer.spatial_k_anonymity(
            self.points_gdf, k=k, h3_resolution=7
        )
    ...
    def test_geographic_masking(self):
        ...
        admin_polygons = [
            ...
            Polygon([(-73.5, 40.0), (-73.0, 40.0), (-73.0, 40.5), (-73.5, 40.5)]),
        ]
        ...
        mixed_gdf = gpd.GeoDataFrame(
            {
                ...
                "geometry": [
                    Point(-73.5, 40.5),
                    Polygon(
                        [(-74.0, 40.0), (-73.5, 40.0), (-73.5, 40.5), (-74.0, 40.5)]
                    ),
                ],
            },
            crs="EPSG:4326",
