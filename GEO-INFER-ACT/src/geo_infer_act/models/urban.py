...
from geo_infer_act.api.interface import ActiveInferenceInterface # Keeping for compatibility if needed, but using Agent directly
...
class UrbanModel(ActiveInferenceModel):
    ...
    def step(self, input_actions=None):
        ...
        for agent_data in self.agents:
            ...
            target_loc = int(action[0]) if isinstance(action, (list, np.ndarray)) else int(action)
