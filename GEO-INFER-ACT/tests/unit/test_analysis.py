...
from geo_infer_act.utils.analysis import (
...
class TestActiveInferenceAnalyzer(unittest.TestCase):
    ...
    def test_analyze_action_selection_patterns(self):
        ...
        self.analyzer.record_step(
            ...
            np.array([1, 0]),
            1,
            {"all_probabilities": np.array([0.3, 0.7])},
            1.0,
        )
        self.analyzer.record_step(
            np.array([0.6, 0.4]),
            np.array([0, 1]),
            2,
            {"all_probabilities": np.array([0.4, 0.6])},
            0.8,
        )
...
class TestVisualizationFunctions(unittest.TestCase):
    ...
    def test_create_shared_visualizations(self):
        ...
        self.assertTrue(
            (
                self.output_dir / "visualizations" / "belief_evolution_heatmap.png"
            ).exists()
        )
