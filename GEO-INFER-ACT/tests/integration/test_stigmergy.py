...
class TestStigmergicCoordination(unittest.TestCase):
    def test_stigmergy_convergence(self):
        ...
        env_engine = EnvironmentalActiveInferenceEngine(
            h3_resolution=7,
            environmental_variables=["vegetation_density", "human_activity"],
        )
        ...
        model = MultiAgentModel(
            n_agents=len(env_engine.environmental_states),
            environmental_engine=env_engine,
        )
        ...
        _history = model.simulate_h3_lattice(
            timesteps=5, obs_gen=env_observation_generator
        )
        ...
        final_activity_sum = sum(
            [
                getattr(state, "human_activity", 0.0)
                for state in env_engine.environmental_states.values()
            ]
        )
        self.assertTrue(
            final_activity_sum > 0.0,
            "Stigmergy failed: Agents did not modify the shared environmental manifold.",
        )
