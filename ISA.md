...
- [ ] Gaussian diagnostics satisfy F = KL - expected log likelihood and agree with analytic evidence after exact filtering; invalid dimensions/covariances fail atomically.
- [ ] H3 state order is stable, sparse movement conserves probability at ordinary/pentagon/boundary cells, and dense conversion is bounded.
- [ ] UTC inference records preserve sequence and reject gaps, duplicates and naive timestamps before execution.
- [ ] GNN exports explicit single-factor A–E artifacts; ACT rejects unsupported semantics, preserves policy priors and conditions/propagates exactly once per timestamp.
- [ ] Separate GNN and GEO environments complete an artifact round trip with a source digest, real pymdp execution and repeatable traces.
- [ ] Existing GNN dirty files retain their original bytes; focused/full affected-module tests and repository contracts pass; remaining scope is concrete in TODO.
