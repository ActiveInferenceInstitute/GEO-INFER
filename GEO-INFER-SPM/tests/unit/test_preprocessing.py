...
class TestDataPreprocessingPipeline:
    ...
    def test_selective_preprocessing(self):
        ...
        # Test multiple steps
        processed = preprocess_data(self.spm_data, steps=["validate", "normalize"])

        assert set(processed.metadata["preprocessing_steps"]) == {
            "validate",
            "normalize",
        }
...
class TestMissingDataHandling:
    ...
    def test_too_many_missing_data(self):
        ...
        spm_data_missing = SPMData(
            data=data_too_many_missing, coordinates=self.coordinates, crs="EPSG:4326"
        )

        with pytest.raises(ValueError, match="Too much missing data"):
            handle_missing_data(
                spm_data_missing, method="interpolate", max_missing_fraction=0.5
            )
...
class TestDataNormalization:
    ...
    def test_robust_normalization(self):
        ...
        spm_data_outliers = SPMData(
            data=data_with_outliers, coordinates=self.coordinates, crs="EPSG:4326"
        )
...
class TestOutlierRemoval:
    ...
    def test_no_outliers(self):
        ...
        clean_spm_data = SPMData(
            data=clean_data, coordinates=coordinates, crs="EPSG:4326"
        )
...
class TestSpatialFiltering:
    ...
    def setup_method(self):
        ...
        self.data = np.sin(self.coordinates[:, 0] / 10) + 0.1 * np.random.randn(
            n_points
        )
...
class TestTemporalFiltering:
    ...
    def setup_method(self):
        ...
        self.coordinates = np.column_stack(
            [np.random.uniform(-179, 179, 1), np.random.uniform(-89, 89, 1)]
    ...
    def test_moving_average_filter(self):
        ...
        processed = temporal_filter(
            self.spm_data, method="moving_average", window_size=5
        )
    ...
    def test_savgol_filter(self):
        ...
        try:
            processed = temporal_filter(
                self.spm_data, method="savitzky_golay", window_size=7
            )
    ...
    def test_temporal_filter_without_time(self):
        ...
        with pytest.raises(
            ValueError, match="Temporal filtering requires time dimension"
        ):
