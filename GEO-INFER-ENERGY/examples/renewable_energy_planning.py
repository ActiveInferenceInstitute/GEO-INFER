...
from geo_infer_energy import RenewableResourceAssessor, RenewableType, RenewableSite
...
def main():
    ...
    print(
        f"   Average annual potential: {float(solar_result['solar_potential'].mean()):.0f} kWh/m²"
    )
    print(
        f"   Average annual energy (20% eff.): {float(solar_result['annual_energy'].mean()):.0f} kWh/m²"
    )

    # 3. Assess Wind Resources
    print("\n3. Assessing Wind Resources...")

    wind_speed = xr.DataArray(np.random.uniform(6, 10, (10, 10)), dims=["lat", "lon"])
    ...
    candidate_sites = [
        ...
        },
        {
            ...
            "type": RenewableType.SOLAR_PV,
            "resource": 6.5,
            "constraints": {"steep_slope": True},
        },
        {
            "id": "WND_01",
            "loc": (-117.5, 34.0),
            "type": RenewableType.ONSHORE_WIND,
            "resource": 8.5,
            "constraints": {},
        },
        {
            ...
            "type": RenewableType.ONSHORE_WIND,
            "resource": 7.0,
            "constraints": {"grid_distance_km": 75},
        },
        {
            "id": "WND_03",
            "loc": (-116.5, 35.5),
    ...
    for site in candidate_sites:
        ...
        print(
            f"   {site['id']:<10} {site['type'].value:<15} {site['resource']:>10.1f} "
    ...
    print(
        f"   {'Onshore Wind':<15} {wind_cf['capacity_factor_pct']:>17.1f}% {wind_cf['annual_generation_gwh']:>18.1f}"
    )
    ...
    gen_profile = xr.DataArray(
        solar_irradiance / 800 * 500 + wind_data.values * 30, dims=["time"]
    )
    ...
    for penetration in [0.3, 0.5, 0.7]:
        ...
        print(
            f"   - Storage energy needed: {storage['recommended_storage']['energy_capacity_mwh']:.0f} MWh"
        )
        print(
            f"   - Curtailment without storage: {storage['curtailment_rate_pct']:.1f}%"
        )
    ...
    for rtype, data in summary["by_resource_type"].items():
        print(
            f"   - {rtype}: {data['count']} sites, {data['capacity_mw']:.0f} MW, {data['generation_gwh']:.1f} GWh"
        )
