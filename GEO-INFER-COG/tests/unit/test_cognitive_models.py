...
class TestSpatialNode:
    ...
    def test_cognitive_weight_default(self) -> None:
        node = SpatialNode(
            node_id="n1", node_type="landmark", saliency=0.8, accessibility=0.9
        )
    ...
    def test_cognitive_weight_with_profile(self) -> None:
        node = SpatialNode(
            node_id="n1", node_type="landmark", saliency=0.5, accessibility=1.0
        )
...
class TestCognitiveMap:
    ...
    def test_add_route_creates_connections(self, cmap: CognitiveMap) -> None:
        cmap.add_landmark(
            "lm_a", {"type": "Point", "coordinates": [0, 0]}, {"name": "A"}
        )
        cmap.add_landmark(
            "lm_b", {"type": "Point", "coordinates": [10, 10]}, {"name": "B"}
        )
        cmap.add_route(
            "r1", "lm_a", "lm_b", [{"segment": 1}], {"length": 100, "mode": "walking"}
        )
    ...
    def test_cognitive_load_increases_with_complexity(self, cmap: CognitiveMap) -> None:
        for i in range(10):
            cmap.add_landmark(
                f"lm_{i}", {"type": "Point", "coordinates": [i, i]}, {"name": f"LM{i}"}
            )
    ...
    def test_export_to_geojson(self, cmap: CognitiveMap) -> None:
        cmap.add_landmark(
            "lm1", {"type": "Point", "coordinates": [1, 2]}, {"name": "Test"}
        )
    ...
    def test_get_map_statistics(self, cmap: CognitiveMap) -> None:
        cmap.add_landmark(
            "lm1", {"type": "Point", "coordinates": [0, 0]}, {"name": "A"}, saliency=0.7
        )
...
class TestSpatialKnowledgeGraph:
    ...
    def test_add_entity(self, skg: SpatialKnowledgeGraph) -> None:
        skg.add_spatial_entity(
            "park_1", "landmark", properties={"name": "Central Park"}
        )
        assert "park_1" in skg.graph
        assert skg.graph_metrics["nodes_count"] == 1

    def test_add_relationship_requires_entities(
        self, skg: SpatialKnowledgeGraph
    ) -> None:
