...
class TestSpatialPercept:
    ...
    def test_default_values(self) -> None:
        percept = SpatialPercept(
            element_id="e1", geometry={"type": "Point", "coordinates": [0, 0]}
        )
    ...
    def test_attention_priority_navigation_boosts_large_scale(self) -> None:
        ...
        assert large.calculate_attention_priority(
            "navigation"
        ) > small.calculate_attention_priority("navigation")

    def test_attention_priority_search_boosts_salient(self) -> None:
        salient = SpatialPercept(
            element_id="e1", geometry={}, visual_saliency=0.9, accessibility=1.0
        )
        dull = SpatialPercept(
            element_id="e2", geometry={}, visual_saliency=0.3, accessibility=1.0
        )
        assert salient.calculate_attention_priority(
            "search"
        ) > dull.calculate_attention_priority("search")
...
class TestAttentionModel:
    ...
    def test_allocate_attention_normalizes(self) -> None:
        ...
        elements = [
            SpatialPercept(
                element_id="a", geometry={}, visual_saliency=0.8, accessibility=1.0
            ),
            SpatialPercept(
                element_id="b", geometry={}, visual_saliency=0.2, accessibility=1.0
            ),
        ]
    ...
    def test_allocate_attention_uniform_when_zero_priority(self) -> None:
        ...
        elements = [
            SpatialPercept(
                element_id="a", geometry={}, visual_saliency=0.0, accessibility=0.0
            ),
            SpatialPercept(
                element_id="b", geometry={}, visual_saliency=0.0, accessibility=0.0
            ),
        ]
        weights = model.allocate_attention(elements)
