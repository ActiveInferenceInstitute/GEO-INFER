...
class TestCognitiveState:
    ...
    def test_add_to_working_memory(self) -> None:
        state = CognitiveState()
        state.add_to_working_memory(
            "location_1", {"lat": 40.0, "lon": -74.0}, importance=0.9
        )
...
class TestCognitiveProcessingEngine:
    ...
    def test_recommend_action_thresholds(self) -> None:
        ...
        strong = engine._recommend_action(
            {"confidence": 0.9}, {"reasoning_path": ["a", "b", "c", "d"]}
        )
        assert strong == "strong_recommendation"

        weak = engine._recommend_action({"confidence": 0.45}, {"reasoning_path": []})
        assert weak == "weak_recommendation"

        further = engine._recommend_action({"confidence": 0.2}, {"reasoning_path": []})
