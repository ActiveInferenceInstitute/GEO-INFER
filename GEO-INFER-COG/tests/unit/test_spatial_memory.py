...
class TestSpatialMemoryItem:
    ...
    def test_default_values(self) -> None:
        item = SpatialMemoryItem(
            item_id="m1", content={"data": 1}, memory_type="working"
        )
...
class TestSpatialMemoryModel:
    ...
    def test_default_memory_types_are_isolated_per_instance(self) -> None:
        first = SpatialMemoryModel()
        second = SpatialMemoryModel()

        first.memory_types.append("semantic")

        assert "semantic" not in second.memory_types
        assert set(second.memory_storage) == set(second.memory_types)
    ...
    def test_capacity_limit_removes_least_important(self) -> None:
        ...
        model.store_spatial_memory(
            content={"id": 1}, memory_type="working", importance=0.3
        )
        model.store_spatial_memory(
            content={"id": 2}, memory_type="working", importance=0.9
        )
        # Third item should trigger removal of least important
        model.store_spatial_memory(
            content={"id": 3}, memory_type="working", importance=0.8
        )
    ...
    def test_search_with_importance_threshold(self) -> None:
        model = SpatialMemoryModel()
        model.store_spatial_memory(
            content={"type": "low"}, memory_type="working", importance=0.2
        )
        model.store_spatial_memory(
            content={"type": "high"}, memory_type="working", importance=0.9
        )
