...
class TestAttentionAllocation:
    ...
    def test_single_element_gets_full_attention(self, model: AttentionModel) -> None:
        elements = [
            SpatialPercept(
                element_id="sole", geometry={}, visual_saliency=0.6, accessibility=1.0
            ),
        ]
    ...
    def test_attention_sums_to_capacity(self, model: AttentionModel) -> None:
        elements = [
            SpatialPercept(
                element_id=f"e{i}",
                geometry={},
                visual_saliency=np.random.random(),
                accessibility=1.0,
            )
            for i in range(5)
        ]
    ...
    def test_reduced_capacity_limits_total(self) -> None:
        ...
        elements = [
            SpatialPercept(
                element_id="a", geometry={}, visual_saliency=0.5, accessibility=1.0
            ),
            SpatialPercept(
                element_id="b", geometry={}, visual_saliency=0.5, accessibility=1.0
            ),
        ]
        weights = model.allocate_attention(elements)
        total = sum(weights.values())
    ...
    def test_high_saliency_dominates(self) -> None:
        ...
        elements = [
            SpatialPercept(
                element_id="high", geometry={}, visual_saliency=0.95, accessibility=1.0
            ),
            SpatialPercept(
                element_id="low", geometry={}, visual_saliency=0.05, accessibility=1.0
            ),
        ]
        weights = model.allocate_attention(elements)
...
class TestAttentionPriority:
    ...
    def test_analysis_context_favors_low_uncertainty(self) -> None:
        ...
        assert certain.calculate_attention_priority(
            "analysis"
        ) > uncertain.calculate_attention_priority("analysis")

    def test_planning_context_is_neutral(self) -> None:
        percept = SpatialPercept(
            element_id="p", geometry={}, visual_saliency=0.6, accessibility=0.8
        )
    ...
    def test_zero_accessibility_means_zero_priority(self) -> None:
        percept = SpatialPercept(
            element_id="z", geometry={}, visual_saliency=1.0, accessibility=0.0
        )
...
class TestPerceptualGrouping:
    ...
    def test_nearby_elements_grouped_together(self) -> None:
        ...
        elements = [
            SpatialPercept(
                element_id="a", geometry={"type": "Point", "coordinates": [0, 0]}
            ),
            SpatialPercept(
                element_id="b", geometry={"type": "Point", "coordinates": [1, 1]}
            ),
        ]
    ...
    def test_distant_elements_not_grouped(self) -> None:
        ...
        elements = [
            ...
            SpatialPercept(
                element_id="b", geometry={"type": "Point", "coordinates": [100, 100]}
            ),
        ]
        grouped = model._apply_perceptual_grouping(elements)
        ...
        assert (
            len(set(groups)) <= 1
        )  # At most one group from one pair, or neither grouped

    def test_single_element_not_grouped(self) -> None:
        model = SpatialPerceptionModel()
        elements = [
            SpatialPercept(
                element_id="solo", geometry={"type": "Point", "coordinates": [5, 5]}
            ),
        ]
        grouped = model._apply_perceptual_grouping(elements)
