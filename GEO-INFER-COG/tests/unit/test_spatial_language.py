...
class TestSpatialLanguageProcessor:
    ...
    def test_extract_coordinate_entities(
        self, processor: SpatialLanguageProcessor
    ) -> None:
    ...
    def test_extract_landmark_entities(
        self, processor: SpatialLanguageProcessor
    ) -> None:
    ...
    def test_geocoding_unknown_returns_none(
        self, processor: SpatialLanguageProcessor
    ) -> None:
        coords = processor._attempt_geocoding("Atlantis", "city")
        assert coords is None

    def test_extract_direction_from_text(
        self, processor: SpatialLanguageProcessor
    ) -> None:
        direction = processor._extract_direction_from_text("north of the river")
        assert direction == "north"

    def test_extract_distance_from_text(
        self, processor: SpatialLanguageProcessor
    ) -> None:
        distance = processor._extract_distance_from_text(
            "the hotel is near the station"
        )
        assert distance == "near"

    def test_extract_spatial_concepts(
        self, processor: SpatialLanguageProcessor
    ) -> None:
    ...
    def test_identify_uncertainty_indicators(
        self, processor: SpatialLanguageProcessor
    ) -> None:
    ...
    def test_process_place_description_returns_keys(
        self, processor: SpatialLanguageProcessor
    ) -> None:
