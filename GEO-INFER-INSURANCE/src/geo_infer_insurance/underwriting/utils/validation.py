    ...
    def validate_application(
        ...
        try:
            ...
            if "property" in application_data:
                property_validation = self._validate_property_data(
            ...
            if "applicant" in application_data:
                applicant_validation = self._validate_applicant_data(
            ...
            if "coverage_requests" in application_data:
                coverage_validation = self._validate_coverage_requests(
                    application_data["coverage_requests"]
                )
    ...
    def validate_policy(self, policy_data: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if "expiration_date" in policy_data:
                date_validation = self._validate_date(
                    policy_data["expiration_date"], "expiration_date"
                )
                errors.extend(date_validation["errors"])
                warnings.extend(date_validation["warnings"])
            ...
            if "total_premium" in policy_data:
                premium_validation = self._validate_premium(
                    policy_data["total_premium"]
                )
        ...
        try:
            ...
            if "claimed_amount" in claim_data:
                amount_validation = self._validate_amount(
                    claim_data["claimed_amount"], "claimed_amount"
                )
            ...
            if "date_of_loss" in claim_data:
                date_validation = self._validate_date(
                    claim_data["date_of_loss"], "date_of_loss"
                )
                errors.extend(date_validation["errors"])
                warnings.extend(date_validation["warnings"])
            ...
            if "reported_date" in claim_data:
                date_validation = self._validate_date(
                    claim_data["reported_date"], "reported_date"
                )
                errors.extend(date_validation["errors"])
                warnings.extend(date_validation["warnings"])
    ...
    def _validate_property_data(
        ...
        if "value" in property_data:
            ...
            if value <= 0:
                ...
                warnings.append(
                    "Very high property value - may require special approval"
                )
    ...
    def _validate_single_coverage_request(
        ...
        for field in required_fields:
            if field not in coverage:
                errors.append(
                    f"Coverage request {index}: required field missing: {field}"
                )
        ...
        if "limit" in coverage:
            ...
            if limit <= 0:
                ...
                warnings.append(
                    f"Coverage request {index}: high limit may require approval"
                )
        ...
        if "deductible" in coverage:
            ...
            if deductible < 0:
                errors.append(
                    f"Coverage request {index}: deductible cannot be negative"
                )
...
class PolicyValidator:
    ...
    def __init__(self) -> None:
        ...
        self.logger = logging.getLogger(
            "geo_infer_insurance.underwriting.policy_validator"
        )
    ...
    def validate_policy_renewal(
        ...
        try:
            ...
            current_expiration = datetime.fromisoformat(
                current_policy["expiration_date"].replace("Z", "+00:00")
            )
            renewal_effective = datetime.fromisoformat(
                renewal_data["effective_date"].replace("Z", "+00:00")
            )
        ...
        try:
            ...
            if "effective_date" in endorsement:
                effective_date = datetime.fromisoformat(
                    endorsement["effective_date"].replace("Z", "+00:00")
                )
                policy_effective = datetime.fromisoformat(
                    policy["effective_date"].replace("Z", "+00:00")
                )

                if effective_date < policy_effective:
                    errors.append(
                        "Endorsement effective date cannot be before policy effective date"
                    )
            ...
            if "premium_change" in endorsement:
                ...
                if (
                    abs(premium_change) > policy.get("total_premium", 0) * 0.5
                ):  # 50% change limit
