...
- H3 resolution-7 hexagons colored by ecoregion
- Volcano markers with threat-level colors and eruption info
- CSZ fault line with M9 probability popup
...
_ECOREGION_COLORS = [
    ...
    "#b7e4c7", "#1e6091", "#2a9d8f", "#457b9d", "#6096ba",
]
    ...
    for group in ["chinook_salmon", "coho_salmon", "steelhead",
        ...
        for entry in salmon_data.get(group, []):
            ...
            if status not in ("Not Listed", "Not Listed (Species of Concern)", ""):
                listed.append({"name": entry["name"], "status": status,
...
def _add_integration_layers(
    ...
    if forest_results and isinstance(forest_results, dict):
        ...
        for cell_id, cell_data in forest_results.items():
            score = float(cell_data.get("score", cell_data) if isinstance(cell_data, dict) else cell_data)
            try:
                boundary = h3lib.cell_to_boundary(cell_id)
                locations = [[lat, lon] for lat, lon in boundary]
                g = int(180 * score)
...
def create_bioregion_map(
    ...
    volcanoes_data = _load_json(volcanoes_path) if volcanoes_path.exists() else {"features": []}
    csz_data = _load_json(csz_path) if csz_path.exists() else {"features": []}
    watersheds_data = _load_json(watersheds_path) if watersheds_path.exists() else {"features": []}
    boundary_data = _load_json(boundary_path) if boundary_path.exists() else {"features": []}
    ...
    if watersheds_data.get("features"):
        ...
        def _watershed_popup(feature: dict) -> folium.Popup:
            ...
            dams_html = "".join(f"<li>{d}</li>" for d in major_dams) if major_dams else "<li>None</li>"
...
<b>⚠️ Cascadia Subduction Zone</b><br>
<small><b>M9 probability (50yr):</b> 37%</small><br>
<small><b>Last full rupture:</b> January 26, 1700 CE</small><br>
<small><b>Rupture length:</b> ~1,100 km</small><br>
<small><b>Expected tsunami:</b> Yes — Pacific coast</small><br>
<small><b>Affected area:</b> Northern CA → BC</small>
</div>""",
                max_width=230,
            ),
        )
    ...
    for feature in volcanoes_data.get("features", []):
        ...
        threat = props.get("threat_level", "Moderate")
        ...
        lahar_html = "".join(f"<li>{d}</li>" for d in lahars) if lahars else "<li>None identified</li>"
...
<small><b>Lahar risk drainages:</b><ul style="margin:2px 0 0 12px">{lahar_html}</ul></small>
    ...
    if integration_results:
        _add_integration_layers(m, h3_data, integration_results)
