...
class GeoInferSurfaceWater:
    ...
    def __init__(self, backend: "CascadianAgriculturalH3Backend") -> None:
    ...
    def get_flowline_network(
        ...
        """Build and return a Cascadia flowline network with specified minimum stream order."""
        return self.data_source.get_flowline_network(
    ...
    def acquire_raw_data(self) -> Path:
        ...
        waterbodies = nhd.get("waterbodies") or gpd.GeoDataFrame(
            geometry=[], crs="EPSG:4326"
        )
        flowlines = nhd.get("flowlines") or gpd.GeoDataFrame(
            geometry=[], crs="EPSG:4326"
        )
    ...
    def _get_analysis_bbox(
        ...
        if not target_hexagons:
            return (-124.8, 40.0, -114.5, 49.0)  # Cascadia default bounds
        polygons = [
    ...
    def run_analysis(
        ...
        if (
            (flowlines_gdf is None or flowlines_gdf.empty)
            and (waterbodies_gdf is None or waterbodies_gdf.empty)
        ):
            logger.warning("No surface water features found in the target area.")
            return {
                hex_id: {
                    "water_body_area_sqkm": 0.0,
        ...
        if waterbodies_gdf is not None and not waterbodies_gdf.empty:
            ...
            intersected_wb = gpd.overlay(
                hex_gdf, waterbodies_gdf, how="intersection"
            )
            ...
            if not intersected_wb.empty:
                ...
                area_by_hex = intersected_wb_proj.groupby("hex_id")[
                    "area_sqkm"
                ].sum()
        ...
        if flowlines_gdf is not None and not flowlines_gdf.empty:
            ...
            if not intersected_fl.empty:
                ...
                length_by_hex = intersected_fl_proj.groupby("hex_id")[
                    "length_km"
                ].sum()
    ...
    def run_final_analysis(self, h3_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for hex_id, items in h3_data.items():
            try:
                ...
                wb = (
                    int((df.get("layer") == "waterbodies").sum())
                    if not df.empty and "layer" in df
                    else 0
                )
                fl = (
                    int((df.get("layer") == "flowlines").sum())
                    if not df.empty and "layer" in df
                    else 0
                )
                max_order = (
                    int(df["stream_order"].max())
                    if not df.empty and "stream_order" in df
                    else 0
                )
                results[hex_id] = {
