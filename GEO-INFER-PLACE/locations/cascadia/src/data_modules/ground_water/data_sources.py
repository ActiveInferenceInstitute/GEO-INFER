...
class CascadianGroundWaterDataSources:
    ...
    def fetch_groundwater_data(self, hexagons: List[str]) -> gpd.GeoDataFrame:
        ...
        for i in range(0, len(hexagons), batch_size):
            ...
            try:
                response = requests.get(self.usgs_nwis_url, params=params, headers=headers, timeout=120)
                ...
                if e.response.status_code == 404:
                    ...
                    logger.error(f"HTTP error fetching data from USGS NWIS for batch {i//batch_size + 1}: {e}")
                continue
