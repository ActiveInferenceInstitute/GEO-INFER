...
class InteractiveVisualizationEngine:
    ...
    def _generate_forest_monitoring_sites(self, forest_data: Optional[Dict[str, Any]] = None) -> List[Dict]:
        """Extract forest monitoring sites from analysis data.
        ...
        if forest_data:
            ...
            inventory = (
                ds.get('data_sources', ds) if isinstance(ds, dict) else {}
            )
            plots = (
            ...
            if plots:
                ...
                for i, plot in enumerate(plots):
                    ...
                    health_map = {'Excellent': 0.9, 'Good': 0.7, 'Fair': 0.5, 'Poor': 0.25}
                    ...
                    sites.append({
                        ...
                        'species_diversity': float(plot.get('understory_diversity', 2.5)),
                        'last_survey': plot.get('survey_date', datetime.now().strftime('%Y-%m-%d')),
                    })
    ...
    def _generate_coastal_monitoring_sites(self, coastal_data: Optional[Dict[str, Any]] = None) -> List[Dict]:
        """Extract coastal monitoring sites from analysis data.
        ...
        if coastal_data:
            infra = (
                coastal_data.get('data_acquisition', coastal_data)
                .get('data_sources', coastal_data) if isinstance(coastal_data, dict) else {}
            )
            infra_items = (
                infra.get('coastal_infrastructure', {}).get('infrastructure', [])
                if isinstance(infra, dict) else []
            )
            if infra_items:
                sites = []
        ...
        coastal_refs = [
            {'name': 'Crescent City Harbor', 'lat': 41.7448, 'lon': -124.1837, 'vuln': 0.72, 'erosion': 1.2, 'exposure': 'High'},
            {'name': 'Battery Point', 'lat': 41.7406, 'lon': -124.2028, 'vuln': 0.55, 'erosion': 0.3, 'exposure': 'Moderate'},
            {'name': 'Pebble Beach', 'lat': 41.7250, 'lon': -124.2150, 'vuln': 0.48, 'erosion': 0.8, 'exposure': 'Moderate'},
            {'name': 'Point St. George', 'lat': 41.7833, 'lon': -124.2567, 'vuln': 0.62, 'erosion': 1.5, 'exposure': 'High'},
            {'name': 'Pelican Bay', 'lat': 41.8000, 'lon': -124.2200, 'vuln': 0.38, 'erosion': 0.4, 'exposure': 'Low'},
            {'name': 'Smith River Mouth', 'lat': 41.9292, 'lon': -124.1972, 'vuln': 0.65, 'erosion': 1.8, 'exposure': 'High'},
            {'name': 'Klamath River Mouth', 'lat': 41.5447, 'lon': -124.0800, 'vuln': 0.70, 'erosion': 2.0, 'exposure': 'High'},
            {'name': 'Enderts Beach', 'lat': 41.6892, 'lon': -124.1556, 'vuln': 0.35, 'erosion': 0.2, 'exposure': 'Low'},
            {'name': 'Wilson Creek Beach', 'lat': 41.6000, 'lon': -124.0900, 'vuln': 0.45, 'erosion': 0.6, 'exposure': 'Moderate'},
            {'name': 'False Klamath Cove', 'lat': 41.6200, 'lon': -124.1100, 'vuln': 0.52, 'erosion': 0.9, 'exposure': 'Moderate'},
        ]
        return [
            {
    ...
    def _generate_fire_monitoring_sites(self, fire_data: Optional[Dict[str, Any]] = None) -> List[Dict]:
        """Extract fire monitoring sites from analysis data.
            ...
            if weather_records:
                ...
                for i, (sid, recs) in enumerate(station_map.items()):
                    ...
                    sites.append({
                        ...
                        'suppression_distance': round(latest.get('suppression_distance', 10.0), 1),
                    })
        ...
        fire_refs = [
            {'name': 'Gasquet RAWS', 'lat': 41.8460, 'lon': -123.9700, 'risk': 0.55, 'fm': 14.0, 'fwi': 45, 'dist': 8.0},
            {'name': 'Hiouchi', 'lat': 41.7940, 'lon': -123.9900, 'risk': 0.48, 'fm': 16.0, 'fwi': 38, 'dist': 5.0},
            {'name': 'Siskiyou Wilderness Edge', 'lat': 41.9100, 'lon': -123.8200, 'risk': 0.72, 'fm': 10.0, 'fwi': 62, 'dist': 18.0},
            {'name': 'Klamath Glen', 'lat': 41.5200, 'lon': -124.0300, 'risk': 0.42, 'fm': 18.0, 'fwi': 30, 'dist': 6.0},
            {'name': 'Big Flat', 'lat': 41.5800, 'lon': -123.8900, 'risk': 0.65, 'fm': 12.0, 'fwi': 55, 'dist': 15.0},
            {'name': 'Patrick Creek', 'lat': 41.8700, 'lon': -123.9200, 'risk': 0.50, 'fm': 15.0, 'fwi': 42, 'dist': 12.0},
            {'name': 'Ship Mountain', 'lat': 41.8300, 'lon': -123.7800, 'risk': 0.68, 'fm': 11.0, 'fwi': 58, 'dist': 20.0},
            {'name': 'Darlingtonia', 'lat': 41.7300, 'lon': -124.1100, 'risk': 0.35, 'fm': 20.0, 'fwi': 25, 'dist': 3.0},
            {'name': 'Redwood NP HQ Area', 'lat': 41.6800, 'lon': -124.1000, 'risk': 0.30, 'fm': 22.0, 'fwi': 20, 'dist': 4.0},
            {'name': 'Smith River NRA', 'lat': 41.8100, 'lon': -123.8600, 'risk': 0.58, 'fm': 13.0, 'fwi': 48, 'dist': 14.0},
        ]
        return [
            {
                'site_id': f'FR_{i+1:03d}',
    ...
    def _generate_community_facilities(self, community_data: Optional[Dict[str, Any]] = None) -> List[Dict]:
        ...
        if community_data:
            ...
            if custom:
                logger.info(f"Using {len(custom)} community facilities from analysis data")
                return custom
        ...
        return [
            ...
            {'name': 'Fred Endert Municipal Pool / Community Center', 'type': 'community', 'lat': 41.7522, 'lon': -124.1975, 'capacity': 200, 'service_area': 15, 'accessibility': 'Good'},
            {'name': 'Gasquet Elementary School', 'type': 'education', 'lat': 41.8485, 'lon': -123.9673, 'capacity': 100, 'service_area': 20, 'accessibility': 'Moderate'},
            {'name': 'Del Norte County Sheriff', 'type': 'emergency', 'lat': 41.7553, 'lon': -124.2003, 'capacity': 50, 'service_area': 60, 'accessibility': 'Good'},
            {'name': 'Crescent Elk Middle School', 'type': 'education', 'lat': 41.7611, 'lon': -124.1928, 'capacity': 500, 'service_area': 20, 'accessibility': 'Good'},
            {'name': 'Del Norte County Library', 'type': 'community', 'lat': 41.7560, 'lon': -124.2005, 'capacity': 100, 'service_area': 40, 'accessibility': 'Good'},
        ]
    ...
    def _generate_h3_integration_grid(self, integration_data: Dict[str, Any]) -> Dict[str, Dict]:
        ...
        for domain_name, domain_spatial in domain_datasets.items():
            for cell_id, cell_info in domain_spatial.get('h3_cells', {}).items():
                ...
                score = cell_info.get('forest_health_score',
                        cell_info.get('vulnerability',
                        cell_info.get('risk_level', 0.5)))
                cell_scores.setdefault(cell_id, []).append(float(score))

        if cell_domains:
            max_domains = max(len(v) for v in cell_domains.values())
