...
class AdvancedDashboard:
    ...
    def _load_location_config_bounds(self) -> None:
        ...
        try:
            ...
            config_path = Path(__file__).parent.parent.parent.parent.parent / 'locations' / 'del_norte_county' / 'config' / 'analysis_config.yaml'
    ...
    def fetch_real_time_data(self) -> Dict[str, Any]:
        ...
        fire_perimeters = self.data_integrator.calfire_client.get_fire_perimeters(start_year=2015)
        ...
        data = {
            ...
            'fire_perimeters': {'success': True, 'geojson': fire_perimeters} if fire_perimeters else {'success': False},
        ...
        try:
            ...
            if data.get('tide_levels', {}).get('success'):
                ...
                tide_cache = {
                    'observations': data['tide_levels'].get('series', {}).get('9419750', {}).get('data', []),
                    'latest':  data['tide_levels'].get('series', {}).get('9419750', {}).get('data', [])[-1] if data['tide_levels'].get('series', {}).get('9419750', {}).get('data') else None
                }
    ...
    def _add_county_boundary(self, m: folium.Map):
        ...
        folium.Polygon(locations=boundary_coords, popup="<b>Del Norte County</b><br>Area: 1,008 sq mi", color='red', weight=3, fill=False).add_to(m)
        ...
        if 'fire_data' in self.dashboard_data and self.dashboard_data['fire_data']['success']:
            for incident in self.dashboard_data['fire_data']['incidents']:
                if incident['lat'] and incident['lon']:
                    color = 'red' if incident['contained'] < 50 else 'orange' if incident['contained'] < 100 else 'green'
                    folium.Marker(
        ...
        if 'fire_perimeters' in self.dashboard_data and self.dashboard_data['fire_perimeters'].get('success'):
            ...
            if geojson.get('features'):
                folium.GeoJson(
                    ...
                    style_function=lambda x: {'color': '#d73027', 'weight': 2, 'fillOpacity': 0.05},
                    tooltip=folium.GeoJsonTooltip(fields=['fire_name', 'fire_year'], aliases=['Name', 'Year'])
         ...
         if 'earthquake_data' in self.dashboard_data and self.dashboard_data['earthquake_data']['success']:
            for eq in self.dashboard_data['earthquake_data']['earthquakes']:
                if eq.get('magnitude'):
                    ...
                    folium.CircleMarker(
                        location=[eq['lat'], eq['lon']], radius=radius,
    ...
    def _add_tide_gauge_layer(self, m: folium.Map):
        ...
        if latest:
            ...
            level = latest.get('v') if isinstance(latest, dict) else latest.get('water_level') if isinstance(latest, dict) else 'N/A'
            ts = latest.get('t') if isinstance(latest, dict) else latest.get('time') if isinstance(latest, dict) else 'N/A'
            folium.Marker(
    ...
    def _add_climate_risk_zones(self, m: folium.Map):
        zones = [
            {'name': 'High Fire Risk', 'bounds': [[41.9, -124.3], [42.0, -123.8], [41.8, -123.7], [41.7, -124.2]], 'color': 'red'},
            {'name': 'Coastal Flood', 'bounds': [[41.85, -124.4], [41.95, -124.3], [41.75, -124.2], [41.65, -124.35]], 'color': 'blue'}
        ]
        for z in zones:
            folium.Polygon(locations=z['bounds'], popup=z['name'], color=z['color'], fill=True).add_to(self.layer_groups['climate'])

    def _add_zoning_overlay(self, m: folium.Map):
        areas = [
            {'name': 'Conservation', 'bounds': [[41.8, -124.2], [42.0, -123.7], [41.9, -123.6], [41.7, -124.1]], 'color': '#228B22'},
        ]
        for a in areas:
            folium.Polygon(locations=a['bounds'], popup=a['name'], color=a['color'], fill=True).add_to(self.layer_groups['zoning'])

    def _add_conservation_areas(self, m: folium.Map):
        # Redwood Parks
        folium.Polygon(
            locations=[[41.3, -124.1], [41.5, -124.0], [41.4, -123.9], [41.2, -124.0]],
            popup="Redwood National and State Parks", color='darkgreen', fill=True
    ...
    def _add_economic_indicators(self, m: folium.Map):
        ...
        for c in centers:
            folium.CircleMarker(location=c['loc'], radius=15, popup=f"{c['name']}: {c['emp']} jobs", color='gold', fill=True).add_to(self.layer_groups['economic'])

    def _add_emergency_services_layer(self, m: folium.Map):
        facilities = [{'loc': [41.7586, -124.2031], 'name': 'Sutter Coast Hospital', 'icon': 'plus', 'color': 'green'}]
        for f in facilities:
            folium.Marker(location=f['loc'], popup=f['name'], icon=folium.Icon(color=f['color'], icon=f['icon'])).add_to(self.layer_groups['emergency'])

    def _add_public_health_layer(self, m: folium.Map):
        folium.Marker(location=[41.7520, -124.2010], popup="Community Health Center", icon=folium.Icon(color='lightgreen', icon='heart')).add_to(self.layer_groups['health'])

    def _add_infrastructure_layer(self, m: folium.Map):
        folium.Marker(location=[41.7450, -124.2370], popup="Crescent City Harbor", icon=folium.Icon(color='darkblue', icon='anchor')).add_to(self.layer_groups['infrastructure'])

    def _add_environmental_justice_layer(self, m: folium.Map):
        folium.Polygon(locations=[[41.80, -124.15], [41.82, -124.12], [41.79, -124.11]], popup="Tribal Lands Impact", color='purple', fill=True).add_to(self.layer_groups['equity'])
