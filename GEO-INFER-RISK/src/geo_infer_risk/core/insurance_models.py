...
class InsuranceModel(ABC):
    ...
    def fit(self, historical_data: pd.DataFrame) -> "InsuranceModel":
        ...
        raise RuntimeError(
            "InsuranceModel.fit must be implemented by a concrete insurance model"
        )
...
class PropertyInsuranceModel(InsuranceModel):
    ...
    def fit(self, historical_data: pd.DataFrame) -> "PropertyInsuranceModel":
        ...
        if "property_type" in historical_data.columns:
            type_stats = (
                historical_data.groupby("property_type")
                .agg({"loss_amount": ["mean", "std", "count"], "premium": "mean"})
                .to_dict()
            )
    ...
    def _calculate_risk_factors(self) -> None:
        ...
        if "location" in self.historical_data.columns:
            location_stats = self.historical_data.groupby("location").agg(
                {"loss_amount": "mean", "claim_frequency": "mean"}
            )
    ...
    def calculate_premium(self, risk_profile: Dict[str, Any]) -> float:
        ...
        base_rate = self.base_rates.get(property_type, {}).get(
            ("premium", "mean"), 0.005
        )
        ...
        location_factor = self._get_location_factor(
            risk_profile.get("location", "unknown")
        )
        construction_factor = self._get_construction_factor(
            risk_profile.get("construction_type", "frame")
        )
    ...
    def estimate_losses(self, risk_profile: Dict[str, Any]) -> Dict[str, float]:
        ...
        # Base loss estimates
        base_loss_rate = self.base_rates.get(property_type, {}).get(
            ("loss_amount", "mean"), 0.02
        )
        ...
        loss_std = self.base_rates.get(property_type, {}).get(
...
class LiabilityInsuranceModel(InsuranceModel):
    ...
    def fit(self, historical_data: pd.DataFrame) -> "LiabilityInsuranceModel":
        ...
        if "business_type" in historical_data.columns:
            business_stats = (
                historical_data.groupby("business_type")
                .agg(
                    {
    ...
    def calculate_premium(self, risk_profile: Dict[str, Any]) -> float:
        ...
        # Base rate per $1000 of coverage
        base_rate = self.claim_frequencies.get(business_type, {}).get(
            ("premium", "mean"), 0.001
        )
        ...
        experience_factor = self._get_experience_factor(
            risk_profile.get("claims_history", [])
        )
    ...
    def estimate_losses(self, risk_profile: Dict[str, Any]) -> Dict[str, float]:
        ...
        # Base loss estimates
        claim_frequency = self.claim_frequencies.get(business_type, {}).get(
            ("claim_frequency", "mean"), 0.01
        )
        claim_severity = self.claim_frequencies.get(business_type, {}).get(
            ("claim_severity", "mean"), 50000
        )
...
class CatastropheInsuranceModel(InsuranceModel):
    ...
    def calculate_premium(self, risk_profile: Dict[str, Any]) -> float:
        ...
        for cat_type in catastrophe_types:
            if cat_type in self.catastrophe_models:
                cat_premium = self.catastrophe_models[cat_type](
                    coverage_limit, location
                )
                total_premium += cat_premium

        # Apply reinsurance costs
        reinsurance_cost = self._calculate_reinsurance_cost(
            total_premium, coverage_limit
        )
        total_premium += reinsurance_cost

        # Apply expense and profit loading
        total_premium = total_premium * (
            1 + self.config.expense_ratio + self.config.profit_margin
        )
    ...
    def estimate_losses(self, risk_profile: Dict[str, Any]) -> Dict[str, float]:
        ...
        for cat_type in catastrophe_types:
            if cat_type in self.catastrophe_models:
                cat_loss = self._estimate_catastrophe_loss(
                    cat_type, coverage_limit, location
                )
    ...
    def _estimate_catastrophe_loss(
        ...
        return {"expected_loss": expected_loss, "max_loss": coverage_limit}

    def _calculate_reinsurance_cost(
        self, base_premium: float, coverage_limit: float
    ) -> float:
        """Calculate reinsurance cost."""
        if coverage_limit <= self.config.reinsurance_attachment:
            return 0.0

        reinsurance_coverage = min(
            coverage_limit - self.config.reinsurance_attachment,
...
def calculate_property_premium(
    ...
    if historical_data is None or historical_data.empty:
        raise ValueError(
            "historical_data is required; premiums must be fitted from observed losses"
        )
