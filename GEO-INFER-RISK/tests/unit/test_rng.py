"""Tests for the RISK random-number-generator resolver.

These pin the contract every stochastic entry point in GEO-INFER-RISK relies
on: a seed always becomes an explicit ``Generator``, the process-wide
``numpy.random`` singleton is never read or advanced, and ``0`` is a real seed.
"""
...
from geo_infer_risk.utils.rng import derive_int_seed, resolve_rng, spawn_rng
