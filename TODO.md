...
| **CODE-01** | Repository / recurring index refresh | [BLOCKED-EXTERN, GEO side feasible] The GNN and GEO GitNexus index artifacts were lost with the deleted scratchpad worktrees and `~/.gitnexus/registry.json` registers neither repository; every SHA the indexes referenced is additionally a pre-rewrite historical identifier. The `gitnexus` CLI is on PATH: run `gitnexus analyze .` in this repository (single bounded command, registers on first run) and record the receipt; the GNN checkout gets the same command in its own live checkout, outside this repository's change scope. | Indexed/current-commit parity plus correct explicit-file Gaussian-exporter and sparse-transition lookups; direct source/caller review remains the fallback while no index exists. |
