...
| **CODE-01** | Repository / recurring index refresh | GNN index rebuilt successfully at merged `903b9c339`; Gaussian and factored exporter lookups resolve to the correct source files. GEO symbol lookup was verified at `2b6d669e`. Refresh GEO after acquisition-worker integration and both indexes after later source changes. | Require indexed/current commit parity plus correct explicit-file Gaussian exporter and sparse-transition lookups; inspect unexpected callers/flows. Use direct source/caller review when an index is stale or unavailable. |
...
| INTEGRATE-GNN-01 | GNN PR 25 merged at `903b9c339`; GEO pins that main revision. Complete GEO PR 8 integration and retain exact main workflow receipts. Original dirty checkouts remain preserved. | Preserve concurrent parser/security/module refactors, review ancestry, rerun the paired contract command at the merged revisions, and verify both remote SHAs. |
