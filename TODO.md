...
| **CODE-01** | Repository / recurring index refresh | GEO and GNN indexes were built and used for impact/change review. Refresh against each subsequently published commit before relying on graph results. | Record indexed repository identity and commit SHA; review unexpected callers/flows. Until then, retain direct source/caller inspection as the documented fallback, with reduced code-intelligence confidence. |
