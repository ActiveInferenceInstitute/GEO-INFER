...
class TestSimulationConfig:
    ...
    def test_custom_config(self):
        ...
        config = SimulationConfig(time_step=0.5, max_time=50.0, random_seed=42)
...
class TestSimulationEngine:
    ...
    def engine(self):
        ...
        config = SimulationConfig(time_step=1.0, max_time=10.0, random_seed=42)
    ...
    def step_func(self):
        ...
        def step(time, state):
            return {"value": state.get("value", 0) + 1, "time": time}
...
class TestCheckpointing:
    ...
    def test_load_checkpoint(self, engine_with_history):
        ...
        try:
            ...
            assert len(new_engine.state_history) == len(
                engine_with_history.state_history
            )
        finally:
            os.unlink(filepath)
